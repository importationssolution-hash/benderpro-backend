// Bender Pro v8.0 — Scan via WebSocket Kraken (quasi instantané)
...
const OpenPositionSchema = new mongoose.Schema({
  ...
  figureTarget: Number, // hauteur mesurée de la figure en %
  qty:          Number,
  amount:       Number,
...
// DÉTECTION DE FIGURES — TP +12% / SL -3%
function detectFigure(closes, volumes, livePrice) {
  ...
  // → Détermine le TP dynamique selon la puissance du signal
  const figureTarget = (h - l) / l; // hauteur relative de la figure

  // Filtre: figure trop petite (<40%) → ignorée
  if (figureTarget < 0.40) return null;

  // TP dynamique selon la hauteur mesurée de la figure
  // SL toujours -3%
  let tpPct;
  if      (figureTarget >= 0.80) tpPct = 0.20; // 80-100% → TP +20%
  else if (figureTarget >= 0.71) tpPct = 0.17; // 71-80%  → TP +17%
  else if (figureTarget >= 0.61) tpPct = 0.15; // 61-70%  → TP +15%
  else if (figureTarget >= 0.50) tpPct = 0.13; // 50-60%  → TP +13%
  else                           tpPct = 0.08; // 40-49%  → TP +8%
...
let ws = null;
let ws4h = null; // WebSocket séparé pour 4h
...
// Prix live par paire — mis à jour par le ticker WebSocket
const livePrices = {};
      ...
      if (msg.channel === 'ticker' && msg.data) {
        for (const t of msg.data) {
          if (t.symbol && t.last) {
            ...
            // Format alternatif BTC↔XBT
            const alt = t.symbol.replace('XBT','BTC').replace('BTC','XBT');
            if (alt !== t.symbol) livePrices[alt] = t.last;

            // ── DÉTECTION DE CASSURE EN TEMPS RÉEL ──
            // Scanner seulement si prix bougé de 0.05% → réactif sans spam
            const lastP = lastScannedPrice[t.symbol];
            const priceMoved = !lastP || Math.abs(t.last - lastP) / lastP > 0.0005;
            if (priceMoved) {
              lastScannedPrice[t.symbol] = t.last;
              setImmediate(() => {
                scanSinglePair(t.symbol, '1d');
                scanSinglePair(t.symbol, '4h');
              });
            }
          }
        }
    ...
    try {
      ...
      if (msg.channel === 'ohlc' && (msg.type === 'snapshot' || msg.type === 'update') && msg.data) {
        for (const c of msg.data) {
          ...
          // Stocker seulement close + volume → 3x moins de RAM
          ...
          // Scanner UNIQUEMENT cette paire immédiatement — pas les 500
          // → détection instantanée sans OOM
...
function connectKrakenWS4h(pairs) {
    ...
    try {
      ...
      if (msg.channel === 'ohlc' && (msg.type === 'snapshot' || msg.type === 'update') && msg.data) {
        for (const c of msg.data) {
          ...
          // Scanner cette paire en 4h immédiatement
  ...
  ws4h.on('close', () => {
    console.log('[WS-4h] Déconnecté — reconnexion dans 5s');
...
async function preloadHistoricalCandles(pairs) {
    ...
    await Promise.all(batch.map(async (symbol) => {
      try {
        ...
        krakenCandles[symbol] = ohlcv.map(c => ({
          c: c[4], v: c[6] || c[5]  // seulement close + volume en mémoire
        }));
        loaded++;
      } catch (e) {}
    }));
  ...
  console.log(`Preloading 4h terminé — ${loaded4h}/${pairs.length} paires`);
}
...
// Anti-spam: éviter de remettre le même signal en cache si déjà présent
const recentSignals = new Map(); // symbol → timestamp dernier signal

// Scan d'UNE seule paire — appelé instantanément sur chaque update WebSocket
function scanSinglePair(symbol, timeframe = '1d') {
  try {
    ...
    // Prix live tick par tick — c'est CE prix qui décide de la cassure
    // pas le close de la dernière bougie
    const livePrice = livePrices[symbol]; // undefined si pas encore reçu
    if (!livePrice) return; // pas de prix live = on skip, pas d'entrée sur données stales
    const sig = detectFigure(closes, volumes, livePrice);
    if (!sig) return;

    // Anti-spam mémoire: évite de spammer executeTrade sur chaque tick WebSocket
    // La vraie logique "une figure = un trade" est dans executeTrade() via la DB
    const sigKey  = symbol + '|' + sig.fig.name + '|' + timeframe;
    const rateKey = symbol + '|' + sig.fig.name + '|raté';
    if (recentSignals.get(rateKey)) return; // signal déjà raté (fonds insuf) — oublié
    ...
    const tpPctSignal = sig.tp / livePrice - 1; // tpPct réel calculé
    const signal = {
      symbol,
      exchange:    'Kraken',
      exchangeId:  'kraken',
    ...
    // Ajouter au cache (remplacer si déjà présent)
    ...
    // Exécuter le trade immédiatement si utilisateurs connectés
    executeTrade(signal).catch(() => {});

    console.log(`[Signal] ${symbol} · ${sig.fig.name} · ${livePrice}`);
  } catch(e) {}
}

// Exécute un trade pour tous les utilisateurs Kraken connectés
async function executeTrade(signal) {
  try {
    const users = await User.find({ active: true, apiKey: { $exists: true }, exchangeName: /kraken/i });
    for (const user of users) {
      try {
        // Règle 1: Pas de position ouverte sur cette paire
        const existingPos = await OpenPosition.findOne({ email: user.email, symbol: signal.symbol });
        if (existingPos) {
          console.log(`[Bot] Position déjà ouverte sur ${signal.symbol} — skip`);
          continue;
        }

        // Règle 2: même figure déjà tradée → skip jusqu'à nouvelle formation
        ...
        if (lastTrade) {
          ...
          if (sameFigure && !newFormation) {
            console.log(`[Bot] ${signal.symbol} — même figure ${signal.figure} déjà tradée, attente nouvelle formation (besoin >20% de diff de prix)`);
            continue;
          }
        }
        ...
        if (usd < amount) {
          console.log(`[Bot] ⚠ ${signal.symbol} — fonds insuffisants (${usd.toFixed(2)}$ dispo / ${amount}$ requis) — position oubliée, next`);
          // Marquer ce signal comme "raté" dans recentSignals pour ne pas réessayer
          // Marquer comme raté pour les deux timeframes
          recentSignals.set(signal.symbol + '|' + signal.figure + '|1d|raté', Date.now());
          recentSignals.set(signal.symbol + '|' + signal.figure + '|4h|raté', Date.now());
          continue; // oubli immédiat
        }
        const qty     = amount / signal.entryPrice;
        const tpPrice = signal.tp;
        const slPrice = signal.sl;
        console.log(`[Bot] ORDRE BUY: ${signal.symbol} · ${signal.figure} · $${amount} · TP:${tpPrice} · SL:${slPrice}`);
        ...
        await new Trade({ email: user.email, symbol: signal.symbol, exchange: 'Kraken',
          ...
          result: 'OPEN', exitReason: 'Position ouverte — en attente TP/SL' }).save();
...
// ══════════════════════════════════════════════════════════════════════
// SUIVI TP/SL INSTANTANÉ — toutes les 2 secondes via prix WebSocket
// ══════════════════════════════════════════════════════════════════════
...
async function checkTPSLInstant() {
  try {
    ...
    for (const pos of positions) {
      ...
      if (!currentPrice) {
        // Log diagnostic si prix manquant — aide à débugger
        const keys = Object.keys(livePrices).slice(0, 3).join(', ');
        console.log(`[TP/SL] ⚠ Prix manquant pour ${pos.symbol} (livePrices a ${Object.keys(livePrices).length} entrées, ex: ${keys})`);
        continue;
      }

      // Validation TP/SL coherents (TP doit etre > entryPrice, SL < entryPrice)
      if (pos.tp <= pos.entryPrice || pos.sl >= pos.entryPrice) {
        console.log(`[INSTANT TP/SL] ⚠ TP/SL incohérents pour ${pos.symbol} — correction auto`);
        ...
        console.log(`[INSTANT TP/SL] Corrigé: TP=${correctedTP} SL=${correctedSL}`);
        continue;
      }
      ...
      try {
        ...
        if (!ExClass) {
          console.log(`[INSTANT TP/SL] Exchange ${exchangeName} non supporté — position supprimée`);
          await OpenPosition.deleteOne({ _id: pos._id });
          positionsInProgress.delete(posId);
          continue;
        }
        ...
        if (baseBalance < 0.000001) {
          // Fonds insuffisants — la position a probablement deja ete vendue manuellement
          console.log(`[INSTANT ${reason}] ${pos.symbol} — solde ${base} insuffisant (${baseBalance}) — position supprimée de la DB`);
        ...
        // Utiliser le tpPct réel de la position si disponible, sinon fallback TP_PCT
        ...
        if (e.message && e.message.includes('Insufficient funds')) {
          console.log(`[INSTANT TP/SL] Fonds insuffisants confirmés — suppression position ${pos.symbol}`);
...
async function scanAll() {
  ...
  try {
    ...
    // Trades exécutés instantanément par executeTrade() sur chaque signal WebSocket
    ...
    for (const user of users) {
      ...
      try {
        ...
        if (usd < amount) {
          console.log(`[Bot] ⚠ Fonds insuffisants (${usd.toFixed(2)} USD) — tous les signaux ignorés pour ${user.email}`);
          continue; // skip immédiat
        }
        let ordersPlaced = 0;
        ...
        for (const sig of userSignals.slice(0, MAX_CONCURRENT)) {
          ...
          try {
            ...
            if (sig.direction === 'Long' && quoteBalance >= amount) {
              // Règle 1: pas de position ouverte
              const existingPos = await OpenPosition.findOne({ email: user.email, symbol: sig.symbol });
              if (existingPos) { console.log(`[Bot] Position ouverte sur ${sig.symbol} — skip`); continue; }

              // Règle 2: même figure déjà tradée → attendre nouvelle formation
              const lastTrade = await Trade.findOne(
              ...
              if (lastTrade) {
                ...
                const newFormation = priceDiff > 0.20;
                if (sameFigure && !newFormation) {
                  console.log(`[Bot] ${sig.symbol} — même figure ${sig.figure} déjà tradée, attente nouvelle formation`);
                  continue;
                }
              }
...
app.get('/', (req, res) => res.json({
  ...
  strategy:           'Figures chartistes + Volume · TP+12% SL-3% · Timeframe 1D',
...
app.post('/connect', async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, message: `Connecte sur ${exchangeName} · $${tradeAmount || TRADE_AMOUNT}/trade · TP+${TP_PCT*100}% SL-${SL_PCT*100}% · Daily 150 bougies` });
  } catch (e) { res.json({ success: false, error: e.message }); }
});
...
// Route positions actives avec prix live intégrés
...
// ══════════════════════════════════════════════════════════════════════
// FACTURATION — Commission 0.25% du volume / 2 semaines
// ══════════════════════════════════════════════════════════════════════
...
// DÉMARRAGE
// ══════════════════════════════════════════════════════════════════════
// Augmenter la limite mémoire Node.js si pas déjà fait
// → Ajouter dans Render: Start Command = node --max-old-space-size=460 server.js
// (460MB = safe pour instance 512MB)

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => {
  console.log(`\n Bender Pro v8.0 · Port ${PORT}`);
  console.log(` Figures chartistes + Volume · TP+${TP_PCT*100}% SL-${SL_PCT*100}% · Daily 150 bougies`);
  console.log(` Scan Kraken via WebSocket (quasi instantane)`);
  console.log(` $${TRADE_AMOUNT}/trade · SL -${SL_PCT*100}% · TP +${TP_PCT*100}%`);
  console.log(` Scan toutes les 60 secondes (bougies 1D)\n`);
  setImmediate(async () => {

    // ── ÉTAPE 1: Démarrer TP/SL IMMÉDIATEMENT (avant tout le reste)
    setInterval(() => checkTPSLInstant().catch(console.error), 2000);
    console.log(' Suivi TP/SL instantané actif (toutes les 2 secondes)');

    // ── ÉTAPE 2: Récupérer les paires Kraken
    krakenPairsList = await fetchKrakenUsdtPairs().catch(() => []);
    if (krakenPairsList.length === 0) {
      console.log('Aucune paire trouvée — retry dans 15s');
      setTimeout(() => initKrakenWS(), 15000);
    } else {
      console.log(`${krakenPairsList.length} paires /USD Kraken trouvées`);

      // ── ÉTAPE 3: Connecter le Ticker IMMÉDIATEMENT (prix live pour TP/SL)
      connectKrakenTicker(krakenPairsList);
      console.log(' Ticker WebSocket connecté — prix live actifs');

      // ── ÉTAPE 4: Connecter les WebSocket OHLC IMMÉDIATEMENT (Daily + 4h)
      connectKrakenWS(krakenPairsList);
      connectKrakenWS4h(krakenPairsList); // 4h en parallèle

      // ── ÉTAPE 5: Preloading en arrière-plan (sans bloquer)
      preloadHistoricalCandles(krakenPairsList).then(() => {
        console.log(' Preloading terminé — signaux historiques disponibles');
        // Premier scan complet après preloading
        scanAll().catch(console.error);
      }).catch(console.error);
    }

    // ── ÉTAPE 6: Scan toutes les 60s (indépendant du preloading)
    setTimeout(() => {
