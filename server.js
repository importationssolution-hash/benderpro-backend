...
// Obtenir les params actifs d'un robot
// Tous les robots utilisent PARAMS_REFERENCE sauf si une amélioration est EN COURS DE TEST
function getRobotParams(robotId) {
  const val = robotValidation[robotId];
  if (!val) return PARAMS_REFERENCE;
  // Seulement pendant un test actif → utiliser params proposés
  if (val.status === 'testing' && val.proposedParams) return val.proposedParams;
  // Sinon TOUJOURS les params de référence stricts
...
function robotSingleAnalysis(robot, symbolOverride) {
  ...
  const symbols = symbolOverride
    ? [symbolOverride]
    : Object.keys(krakenCandles).slice(0, 5); // Max 5 paires par robot
...
// Lancer l'analyse des 1000 robots toutes les 30 minutes
// Chaque robot analyse une portion des paires
async function runRobotAnalyses() {
  if (Object.keys(krakenCandles).length < 10) return;

  const startTime = Date.now();
  const robots = Object.values(robotStates);
  const symbols = Object.keys(krakenCandles);
  const analyses = [];

  // Distribuer les paires entre les 1000 robots
  // Chaque robot analyse ~500/1000 = 0.5 paires en moyenne
  // On fait tourner en batch de 50 robots
  const BATCH = 50;
  for (let i = 0; i < robots.length; i += BATCH) {
    const batch = robots.slice(i, i + BATCH);
    await new Promise(r => setImmediate(r)); // CPU breath

    for (const robot of batch) {
      // Chaque robot analyse les paires assignées selon son ID
      const startIdx = (robot.id - 1) % symbols.length;
      const pairsToAnalyze = symbols.slice(startIdx, startIdx + 1)
        .concat(symbols.slice(0, Math.max(0, 1 - (symbols.length - startIdx))));
  ...
  latestRobotAnalyses = analyses.slice(0, 100); // Garder top 100
  lastAnalysisTime = new Date();

  const duration = ((Date.now() - startTime) / 1000).toFixed(1);
  const activeRobots = robots.filter(r => r.status === 'active').length;
  console.log(`[Robots] 📊 ${activeRobots}/1000 robots · ${analyses.length} signaux · ${duration}s`);
...
app.listen(PORT, () => {
  ...
  console.log(` Mode ANALYSE PURE — Trading en pause`);
