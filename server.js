...
const ROBOT_PERSONALITIES = [
  // Agressifs (haut risque, haut rendement)
  { id:1,  name:'Alpha',   sl:0.06, tpMult:1.5, tradeAmount:10, style:'Agressif'    },
  { id:2,  name:'Titan',   sl:0.06, tpMult:1.4, tradeAmount:10, style:'Agressif'    },
  { id:3,  name:'Fury',    sl:0.05, tpMult:1.4, tradeAmount:8,  style:'Agressif'    },
  { id:4,  name:'Blaze',   sl:0.05, tpMult:1.3, tradeAmount:8,  style:'Agressif'    },
  { id:5,  name:'Storm',   sl:0.06, tpMult:1.5, tradeAmount:10, style:'Agressif'    },

  // Équilibrés (notre configuration actuelle)
  { id:6,  name:'Bender',  sl:0.04, tpMult:1.0, tradeAmount:5,  style:'Équilibré'   },
  { id:7,  name:'Nova',    sl:0.04, tpMult:1.1, tradeAmount:5,  style:'Équilibré'   },
  { id:8,  name:'Orion',   sl:0.04, tpMult:1.0, tradeAmount:6,  style:'Équilibré'   },
  { id:9,  name:'Vega',    sl:0.04, tpMult:1.1, tradeAmount:6,  style:'Équilibré'   },
  { id:10, name:'Axon',    sl:0.04, tpMult:1.0, tradeAmount:5,  style:'Équilibré'   },
  { id:11, name:'Zara',    sl:0.035,tpMult:1.0, tradeAmount:5,  style:'Équilibré'   },
  { id:12, name:'Echo',    sl:0.045,tpMult:1.1, tradeAmount:7,  style:'Équilibré'   },
  { id:13, name:'Pulse',   sl:0.04, tpMult:1.2, tradeAmount:5,  style:'Équilibré'   },
  { id:14, name:'Flux',    sl:0.04, tpMult:1.0, tradeAmount:6,  style:'Équilibré'   },
  { id:15, name:'Drift',   sl:0.045,tpMult:1.1, tradeAmount:5,  style:'Équilibré'   },

  // Conservateurs (bas risque, rendement stable)
  { id:16, name:'Shield',  sl:0.02, tpMult:0.8, tradeAmount:5,  style:'Conservateur'},
  { id:17, name:'Sage',    sl:0.02, tpMult:0.8, tradeAmount:5,  style:'Conservateur'},
  { id:18, name:'Calm',    sl:0.025,tpMult:0.9, tradeAmount:5,  style:'Conservateur'},
  { id:19, name:'Stone',   sl:0.025,tpMult:0.9, tradeAmount:5,  style:'Conservateur'},
  { id:20, name:'Vault',   sl:0.02, tpMult:0.8, tradeAmount:5,  style:'Conservateur'},
  { id:21, name:'Zen',     sl:0.03, tpMult:0.9, tradeAmount:5,  style:'Conservateur'},
  { id:22, name:'Anchor',  sl:0.025,tpMult:0.85,tradeAmount:5,  style:'Conservateur'},
  { id:23, name:'Guard',   sl:0.02, tpMult:0.8, tradeAmount:5,  style:'Conservateur'},
  { id:24, name:'Steady',  sl:0.03, tpMult:0.9, tradeAmount:5,  style:'Conservateur'},
  { id:25, name:'Pillar',  sl:0.025,tpMult:0.85,tradeAmount:5,  style:'Conservateur'},

  // Spéciaux (stratégies uniques)
  { id:26, name:'Ghost',   sl:0.03, tpMult:2.0, tradeAmount:3,  style:'Spécial'     }, // Petit montant gros TP
  { id:27, name:'Sniper',  sl:0.01, tpMult:3.0, tradeAmount:3,  style:'Spécial'     }, // SL ultra serré, TP géant
  { id:28, name:'Ninja',   sl:0.05, tpMult:1.8, tradeAmount:4,  style:'Spécial'     },
  { id:29, name:'Phoenix', sl:0.04, tpMult:1.5, tradeAmount:5,  style:'Spécial'     },
  { id:30, name:'Legend',  sl:0.03, tpMult:2.5, tradeAmount:3,  style:'Spécial'     },
];
...
ROBOT_PERSONALITIES.forEach(r => {
  robotStates[r.id] = {
    ...
    status: 'active',         // active | bench | observation
...
let robotQueue = ROBOT_PERSONALITIES.map(r => r.id); // 1,2,3...30
...
// ── Calcul du salaire (montant de trade) selon les points QI
function getRobotTradeAmount(robot) {
  const personality = ROBOT_PERSONALITIES.find(r => r.id === robot.id);
  const base = personality.tradeAmount;

  if (robot.qi >= 500)  return base * 3.0;  // Elite
  if (robot.qi >= 300)  return base * 2.0;  // Expert
  if (robot.qi >= 200)  return base * 1.5;  // Confirmé
  if (robot.qi >= 100)  return base * 1.0;  // Normal
  if (robot.qi >= 50)   return base * 0.7;  // Dégradé
  if (robot.qi >= 0)    return base * 0.5;  // Avertissement
  return 0; // Négatif → banc
}
...
// Historique de validation par robot
const robotValidation = {};
ROBOT_PERSONALITIES.forEach(r => {
  robotValidation[r.id] = {
    currentParams: { ...PARAMS_REFERENCE },  // Params actifs (commence aux params référence)
    proposedParams: null,                     // Params proposés en test
    testTrades: [],                           // Trades sur params proposés
    refTrades: [],                            // Trades sur params référence
    refWinrate: null,                         // Winrate de référence établi
    testWinrate: null,                        // Winrate en test
    status: 'reference',                      // reference | testing | validated | rejected
    testStartDate: null,
...
// Un robot propose une amélioration de paramètres
function proposeImprovement(robotId, newParams) {
  const val = robotValidation[robotId];
  const state = robotStates[robotId];
  if (!val || !state) return { success: false, error: 'Robot non trouvé' };
  if (val.status === 'testing') return { success: false, error: 'Test déjà en cours' };

  // Établir le winrate de référence sur les 20 derniers trades
  const refWinrate = calcWinrate(val.refTrades.slice(-20));
  if (!refWinrate) return { success: false, error: `Pas assez de trades de référence (${val.refTrades.length}/20)` };

  // Valider que les paramètres proposés sont dans des limites raisonnables
  if (newParams.rsiMin < 25 || newParams.rsiMin > 45) return { success: false, error: 'RSI min hors limites (25-45)' };
  if (newParams.volMultiplier < 1.3 || newParams.volMultiplier > 3.0) return { success: false, error: 'Volume mult hors limites (1.3-3.0)' };
  if (newParams.adxMin < 15 || newParams.adxMin > 35) return { success: false, error: 'ADX min hors limites (15-35)' };

  val.proposedParams = { ...PARAMS_REFERENCE, ...newParams };
  val.testTrades = [];
  val.refWinrate = refWinrate;
  val.testWinrate = null;
  val.status = 'testing';
  val.testStartDate = new Date();

  console.log(`[Validation] 🧪 ${state.name} teste nouveaux params · Référence WR: ${refWinrate}% · Besoin: ${(refWinrate + 1).toFixed(1)}%+`);
  return { success: true, refWinrate, required: +(refWinrate + 1).toFixed(1) };
}

// Enregistrer le résultat d'un trade pour la validation
function recordTradeForValidation(robotId, result) {
  const val = robotValidation[robotId];
  if (!val) return;

  if (val.status === 'testing') {
    // En phase de test — enregistrer pour les params proposés
    val.testTrades.push(result);

    // Après 20 trades de test → évaluer
    if (val.testTrades.length >= 20) {
      const testWinrate = calcWinrate(val.testTrades);
      val.testWinrate = testWinrate;
      evaluateImprovement(robotId);
    }
  } else {
    // En phase de référence — enregistrer pour les params actuels
    val.refTrades.push(result);
    if (val.refTrades.length > 50) val.refTrades.shift(); // garder 50 derniers
  }
}

// Évaluer si l'amélioration est prouvée
function evaluateImprovement(robotId) {
  const val = robotValidation[robotId];
  const state = robotStates[robotId];
  if (!val || !state) return;

  const improvement = val.testWinrate - val.refWinrate;
  const improvedPct = Math.floor(improvement); // Arrondir au % inférieur

  if (improvement >= 1.0) {
    // ✅ AMÉLIORATION PROUVÉE
    val.status = 'validated';
    val.currentParams = { ...val.proposedParams };
    val.refTrades = [...val.testTrades]; // Reset référence avec nouveaux trades
    val.testTrades = [];

    // Récompenser le robot
    const qiReward = improvedPct * 1000;
    state.qi += qiReward;

    val.validationHistory.push({
      date: new Date(),
      oldWinrate: val.refWinrate,
      newWinrate: val.testWinrate,
      improvement: +improvement.toFixed(1),
      qiEarned: qiReward,
      params: val.currentParams
    });

    console.log(`[Validation] ✅ ${state.name} AMÉLIORATION PROUVÉE!`);
    console.log(`[Validation]    WR: ${val.refWinrate}% → ${val.testWinrate}% (+${improvement.toFixed(1)}%)`);
    console.log(`[Validation]    +${qiReward} Bender QI points (${improvedPct} × 1000)`);
    console.log(`[Validation]    Nouveaux params adoptés définitivement`);

    // Après validation → repasser en mode référence avec nouveaux params
    setTimeout(() => { val.status = 'reference'; }, 1000);

  } else {
    // ❌ AMÉLIORATION NON PROUVÉE
    val.status = 'rejected';
    val.proposedParams = null;
    val.testTrades = [];

    console.log(`[Validation] ❌ ${state.name} amélioration non prouvée`);
    console.log(`[Validation]    WR test: ${val.testWinrate}% vs référence: ${val.refWinrate}% (${improvement.toFixed(1)}%)`);
    console.log(`[Validation]    Retour aux params de référence`);

    setTimeout(() => { val.status = 'reference'; }, 1000);
  }
}

// Obtenir les params actifs d'un robot (référence ou test)
function getRobotParams(robotId) {
  const val = robotValidation[robotId];
  if (!val) return PARAMS_REFERENCE;
  if (val.status === 'testing' && val.proposedParams) return val.proposedParams;
  return val.currentParams || PARAMS_REFERENCE;
}
...
// Chaque robot analyse toutes les paires et trouve le meilleur signal
function robotSingleAnalysis(robot) {
  let bestSignal = null;
  let bestScore = 0;

  const params = getRobotParams(robot.id);
  const symbols = Object.keys(krakenCandles).slice(0, 200);
...
// Lancer l'analyse des 30 robots toutes les 30 minutes
async function runRobotAnalyses() {
  if (Object.keys(krakenCandles).length < 10) return;

  const analyses = [];
  for (const robot of Object.values(robotStates)) {
    await new Promise(r => setImmediate(r)); // laisser respirer le CPU
    const analysis = robotSingleAnalysis(robot);
    if (analysis) analyses.push(analysis);
  }

  // Trier par score décroissant
  analyses.sort((a, b) => b.score - a.score);
  latestRobotAnalyses = analyses;
  lastAnalysisTime = new Date();

  // Log les 3 meilleures
  const top3 = analyses.slice(0, 3);
  if (top3.length > 0) {
    console.log(`[Robots] 📊 Analyses mises à jour — ${analyses.length} signaux`);
    top3.forEach((a, i) => {
      console.log(`[Robots] ${i+1}. Robot ${a.robot} · ${a.symbol} · ${a.figure} · Score: ${a.score}/100 · ${a.recommendation}`);
    });
  }
}
...
async function runBacktest() {
  ...
  console.log(`[Backtest] 🏆 Paramètres optimaux:`);
  console.log(`[Backtest]   RSI: ${bestParams.rsiMin}-${bestParams.rsiMax}`);
  console.log(`[Backtest]   Volume: ${bestParams.volMultiplier}x`);
  console.log(`[Backtest]   ADX min: ${bestParams.adxMin}`);
  console.log(`[Backtest]   SL: -${(bestParams.slPct*100).toFixed(0)}%`);
  console.log(`[Backtest]   Winrate historique: ${bestParams.winrate}%`);
  console.log(`[Backtest]   Sur ${bestParams.trades} trades simulés\n`);

  // Sauvegarder dans MongoDB pour persistence
  await AIMemory.findOneAndUpdate(
    { email: 'backtest_global' },
    {
      email: 'backtest_global',
      params: {
        rsiMin: bestParams.rsiMin,
        rsiMax: bestParams.rsiMax,
        volMultiplier: bestParams.volMultiplier,
      },
      winRate: bestParams.winrate,
      totalTrades: bestParams.trades,
      lastLearning: new Date(),
      lessons: [`Backtest ${new Date().toLocaleDateString('fr-CA')} · WR:${bestParams.winrate}% · ${bestParams.trades} trades`]
    },
    { upsert: true, new: true }
  ).catch(() => {});
}
...
// Recalibration hebdomadaire — chaque dimanche minuit UTC
function scheduleWeeklyBacktest() {
  ...
  setTimeout(() => {
    console.log('[Backtest] 📅 Recalibration hebdomadaire...');
    runBacktest().catch(console.error);
    setInterval(() => runBacktest().catch(console.error), 7 * 24 * 60 * 60 * 1000);
  }, msUntilSunday);

  console.log(`[Backtest] Prochaine recalibration: ${nextSunday.toLocaleDateString('fr-CA')}`);
}
...
app.get('/bender-qi/status', (req, res) => {
  ...
  const leader = getBenderQILeaderboard()[0];
  res.json({
    success: true,
    totalRobots: 30,
...
app.get('/bender-qi/analyses/:symbol', (req, res) => {
  ...
  const analyses = robotAnalyses[symbol] || [];
  const consensus = getRobotConsensus(symbol);
  res.json({ success: true, symbol, analyses: analyses.slice(-30), consensus });
});
...
app.get('/bender-qi/consensus', (req, res) => {
  const symbols = Object.keys(robotAnalyses);
  const consensuses = symbols.map(s => getRobotConsensus(s)).filter(Boolean);
  const strongSignals = consensuses.filter(c => c.strongSignal);
  res.json({ success: true, total: consensuses.length, strongSignals, consensuses });
});

// Route validation scientifique
app.get('/bender-qi/validation/:id', (req, res) => {
  const val = robotValidation[parseInt(req.params.id)];
  const state = robotStates[parseInt(req.params.id)];
  if (!val || !state) return res.json({ success: false, error: 'Non trouvé' });
  res.json({
    success: true,
    robot: state.name,
    status: val.status,
    refWinrate: val.refWinrate,
    testWinrate: val.testWinrate,
    refTrades: val.refTrades.length,
    testTrades: val.testTrades.length,
    currentParams: val.currentParams,
    validationHistory: val.validationHistory.slice(-5)
  });
});
