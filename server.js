...
function detectFigure(closes, volumes, livePrice) {
  ...
  // Figure valide si hauteur >= 40% — pas de limite de gain max!
  // Si la figure montre 1000% de potentiel → on prend 1000%
  if (figureTarget < 0.40) return null; // trop faible — moins de 40% de hausse potentielle

  // TP = 25% de la hauteur de la figure
  // Exemple: figure de 66% → TP = 66% × 25% = +16.5%
  const tpPct = +(figureTarget * 0.25).toFixed(4);
  function buildLevels() {
    // TP = 25% de la hauteur de la figure au-dessus du prix d'entrée
    const tp = +(price * (1 + tpPct)).toFixed(8);

    // SL = bas de la figure - 0.5% buffer
    // low = plus bas des 500 dernières bougies (bas de la figure)
    const sl = +(low * 0.995).toFixed(8);

    // SL en % par rapport au prix d'entrée (pour affichage)
    const slPctDynamic = +((price - sl) / price * 100).toFixed(2);

    return { tp, sl, slPct: slPctDynamic };
  }
  // DÉTECTION EN FORMATION — prix peut être sous la résistance
  // C'est scanSinglePair qui gère l'attente et l'entrée à la cassure

  if (n >= 50) {
    // CUP & HANDLE: creux profond suivi d'un handle compact près de la résistance
  ...
  if (n >= 50) {
    // ETE INVERSE: tête plus basse + prix remonte vers la ligne de cou
  ...
  // TRIANGLE ASCENDANT: range serré + tendance + prix proche du plus haut
  ...
  // DRAPEAU HAUSSIER: forte montée suivie d'une consolidation
  ...
  // BISEAU BAISSIER: légère baisse puis remontée
    ...
    try {
      ...
      if (msg.channel==='ohlc' && (msg.type==='snapshot'||msg.type==='update') && msg.data) {
        for (const c of msg.data) {
          ...
          // Scan 4h désactivé — Daily uniquement
        }
      }
    } catch(e) {}
  });
  ws4h.on('close', () => { /* 4h désactivé */ });
...
async function preloadHistoricalCandles(pairs) {
  ...
  for (let i=0; i<pairs.length; i+=BATCH) {
    ...
    await Promise.all(batch.map(async (symbol) => {
      ...
      // Preload 4h désactivé — Daily uniquement
    }));
...
function scanSinglePair(symbol, timeframe='1d', exchangeId='kraken') {
  try {
    ...
    // Résistance retournée par la figure ou fallback
    const resistance = sig.resistance || Math.max(...closes.slice(-30));
    const prixVsResistance = (livePrice - resistance) / resistance;

    // Trop tard — prix déjà +4% au-dessus de la résistance
    if (prixVsResistance > 0.04) {
      console.log(`[Figure] ${symbol} · ${sig.fig.name} · Trop tard (+${(prixVsResistance*100).toFixed(1)}%) — skip`);
      return;
    }

    // Cassure fraîche (0% à +4%) → ORDRE IMMÉDIAT
    if (prixVsResistance >= 0) {
      console.log(`[Figure] ${symbol} · ${sig.fig.name} · CASSURE +${(prixVsResistance*100).toFixed(1)}% → ORDRE IMMÉDIAT`);
      executeTrade(signal).catch(() => {});
      return;
    }

    // Prix sous la résistance → en attente de cassure
    if (!breakoutConfirm[symbol] || breakoutConfirm[symbol].figure !== sig.fig.name) {
      ...
      console.log(`[Figure] ${symbol} · ${sig.fig.name} · Attente cassure ${resistance.toFixed(4)} · Prix: ${livePrice.toFixed(4)} (${(prixVsResistance*100).toFixed(1)}%)`);
    }
    // Expiration 7 jours
    setTimeout(() => {
      if (breakoutConfirm[symbol] && breakoutConfirm[symbol].figure === sig.fig.name)
    ...
    for (const user of users.filter(u=>u.botMode==='classic'||!u.botMode)) {
      ...
      try {
        ...
        for (const sig of userSignals.slice(0,MAX_CONCURRENT)) {
          // Vérifier les fonds pour CE signal précis — si insuffisant, signal ignoré (plus favorable)
          ...
          if (freshAvailable < amount) {
            console.log(`[Classic] ${sig.symbol} · Fonds insuffisants (${freshAvailable.toFixed(2)}${currency} / ${amount}${currency}) — signal ignoré, plus favorable`);
          ...
          try {
            ...
            if (recentTrade) continue;
            // Vérifier que le prix actuel est dans la fenêtre de cassure (0% à +4%)
            ...
            if (pctAboveRes > 0.04) {
              console.log(`[Classic] ${sig.symbol} · Trop tard (+${(pctAboveRes*100).toFixed(1)}% > +4%) — skip`);
              continue;
            }
            if (pctAboveRes < 0) {
              console.log(`[Classic] ${sig.symbol} · Pas encore cassé (${(pctAboveRes*100).toFixed(1)}%) — skip`);
              continue;
            }
...
function watchWebSocket() {
  ...
  if (!ws4hOk) {
    ...
    // connectKrakenWS4h désactivé — Daily uniquement
  }
...
app.listen(PORT, () => {
  ...
  setImmediate(async () => {
    ...
    if (krakenPairsList.length > 0) {
      connectKrakenTicker(krakenPairsList);
      connectKrakenWS(krakenPairsList);
      // WS 4h désactivé — bot trade uniquement en Daily
      // connectKrakenWS4h(krakenPairsList); // désactivé
