...
function connectKrakenTicker(pairs) {
  ...
  wsTicker.on('pong', () => {}); // Keepalive TCP
...
function connectKrakenWS(pairs) {
  ...
  ws.on('close',()=>{wsConnected=false;if(ws._hb)clearInterval(ws._hb);setTimeout(()=>connectKrakenWS(krakenPairsList),5000);});
  ws.on('pong', () => {}); // Keepalive TCP
...
app.listen(PORT, () => {
  ...
  setImmediate(async () => {
    ...
    // ── PING GLOBAL — toutes les 15s (Kraken timeout = 60s)
    setInterval(() => {
      const ping = JSON.stringify({ method: 'ping' });
      [wsTicker, ws, ws1h, ws4h].forEach(w => {
        if (w && w.readyState === 1) {
          try { w.send(ping); } catch(e) {}
        }
      });
    }, 15000);
    ...
    setInterval(()=>{
      ...
      if(!wsTicker||wsTicker.readyState!==1){console.log('[KeepAlive] Ticker mort → reconnexion');connectKrakenTicker(krakenPairsList);}
      if(!ws||ws.readyState!==1){console.log('[KeepAlive] Daily mort → reconnexion');connectKrakenWS(krakenPairsList);}
      if(!ws1h||ws1h.readyState!==1){console.log('[KeepAlive] 1H mort → reconnexion');connectKrakenWS1h(krakenPairsList);}
      if(!ws4h||ws4h.readyState!==1){console.log('[KeepAlive] 4H mort → reconnexion');connectKrakenWS4h(krakenPairsList);}
    }, 20000);
