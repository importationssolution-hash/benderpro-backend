...
const bcrypt = require('bcryptjs'); // implementation pure JS, pas de compilation native -- plus fiable sur Render
...
app.post('/create-checkout-session', async (req, res) => {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      mode: 'subscription',
      // Managed Payments (active par defaut sur les comptes Stripe recents) exige
      // un code de taxe produit qu'on n'a pas configure -- on le desactive pour
      // cette session precise, comme suggere par l'erreur Stripe elle-meme.
      // Une fois desactive, on redevient responsable de choisir les methodes de
      // paiement nous-memes -- d'ou payment_method_types ci-dessous.
...
// ================================================================
// AUTHENTIFICATION PAR MOT DE PASSE (Profil)
// Le mot de passe n'est jamais stocke en clair -- seulement son hash
// (bcrypt, avec sel automatique). Meme en cas de fuite de la base de
// donnees, les mots de passe originaux restent proteges.
// ================================================================
app.post('/auth/register', async (req, res) => {
  try {
    const { email, password } = req.body;
    if (!email || !email.includes('@')) return res.json({ success: false, error: 'Email invalide.' });
    if (!password || password.length < 6) return res.json({ success: false, error: 'Mot de passe trop court (6 caractères minimum).' });

    const existing = await User.findOne({ email });
    if (existing && existing.passwordHash) {
      return res.json({ success: false, error: 'Un compte existe déjà avec cet email. Connecte-toi plutôt.' });
    }
...
app.post('/auth/login', async (req, res) => {
  try {
    ...
    if (!user || !user.passwordHash) {
      return res.json({ success: false, error: 'Aucun compte trouvé pour cet email.' });
    }
    const match = await bcrypt.compare(password, user.passwordHash);
    if (!match) return res.json({ success: false, error: 'Mot de passe incorrect.' });
