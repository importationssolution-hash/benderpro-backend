...
// ════════════════════════════════════════════════════════════
// CONFIG GLOBALE — Paramètres stricts fixes
// ════════════════════════════════════════════════════════════
...
const SENTIMENT_MIN        = 60;     // 60% marché vert requis
const ENTRY_MAX_PCT        = 0.03;   // max 3% au-dessus résistance
const BLOCK_HOURS          = 100;    // blocage 100h après trade
...
const TradeSchema = new mongoose.Schema({
  ...
  resistanceBreakAt: Date,   // Quand la résistance a été cassée
...
// ════════════════════════════════════════════════════════════
// FIGURES CHARTISTES — Paramètres fixes stricts
// ════════════════════════════════════════════════════════════
const FIGURES = [
...
// Durée estimée par timeframe
function estimateDuration(timeframe) {
  if (timeframe === '1h') return '2 à 8 heures';
  if (timeframe === '4h') return '1 à 4 jours';
  return '5 à 20 jours';
}

function detectFigure(closes, volumes, livePrice) {
  if (closes.length < 50) return null;
  const RSI_MIN = 33, RSI_MAX = 72;
...
// ════════════════════════════════════════════════════════════
// WEBSOCKET KRAKEN — 4 connexions permanentes
// ════════════════════════════════════════════════════════════
const krakenCandles   = {};
...
// Sentiment du marché — % de paires en hausse
...
// Temps restant avant expiration du signal (filtre 3%)
function getTimeBeforeExpiry(resistanceBreakAt, timeframe) {
  if (!resistanceBreakAt) return null;
  const tfMs = timeframe === '1h' ? 3600000 : timeframe === '4h' ? 14400000 : 86400000;
...
function getMarketScore(symbol, closes, volumes, livePrice) {
  ...
  return { score, rsi: +rsi.toFixed(1), adx: +adx.toFixed(1), volRatio: +volRatio.toFixed(2), sentiment, ema: ema20 > ema50 ? 'Haussière' : 'Baissière' };
}
...
async function runRobotAnalysis() {
  ...
  for (let i = 0; i < symbols.length; i++) {
    ...
    analyses.push({
      ...
      recommendation: metrics.score >= 80 ? '🟢 FORT' : metrics.score >= 60 ? '🟡 MODÉRÉ' : '🔴 FAIBLE',
...
function scanSinglePair(symbol, timeframe='1d', exchangeId='kraken', candleStoreOverride=null) {
  try {
    ...
    if (!breakoutConfirm[bcKey] || breakoutConfirm[bcKey].figure !== sig.fig.name) {
      ...
      console.log(`[Figure ${timeframe}] ${symbol} · ${sig.fig.name} · Résistance ${resistance.toFixed(4)}`);
    }
    setTimeout(() => {
...
// ════════════════════════════════════════════════════════════
// TP/SL — Vérification toutes les 2 secondes
// ════════════════════════════════════════════════════════════
      ...
      try {
        ...
        await Trade.findOneAndUpdate({email:pos.email,symbol:pos.symbol,result:'OPEN'},
          {exitPrice:currentPrice,pnl,result:hitTP?'WIN':'LOSS',
           exitReason:hitTP?`TP +${pos.tpPct}% atteint`:'SL -4% touché'},{sort:{time:-1}});
        await OpenPosition.deleteOne({_id:pos._id});
    ...
    for (const pos of positions) {
      ...
      if (ageHours > 240) {
        ...
        if (curP) {
          ...
          await Trade.findOneAndUpdate({email:pos.email,symbol:pos.symbol,result:'OPEN'},
            {exitPrice:curP,pnl:+pnl.toFixed(4),result:pnl>=0?'WIN':'LOSS',exitReason:'Position fantôme >10j'},{sort:{time:-1}}).catch(()=>{});
          await OpenPosition.deleteOne({_id:pos._id}).catch(()=>{});
        }
      }
    }
...
async function watchDatabase() {
  if (mongoose.connection.readyState !== 1) {
    watchAlert('DATABASE','MongoDB déconnecté');
...
// Route analyse du robot — pour le profil admin
...
app.listen(PORT, () => {
  ...
  console.log(` 1 Robot · Paramètres fixes · SL -4% · Sentiment 60%\n`);
  ...
  setImmediate(async () => {
    ...
    if (krakenPairsList.length > 0) {
      ...
      preloadHistoricalCandles(krakenPairsList).then(() => {
        console.log(' Preloading terminé — Analyse lancée');
