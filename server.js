...
function generateAIStrategy(version) {
  return {
    ...
    cooldown:       0, // IA 100% autonome — pas de cooldown forcé
  };
}
...
function aiScore(closes, volumes, strategy, livePrice) {
  ...
  // Seuil: score doit être positif (l'IA cherche des opportunités)
  // Threshold = 0.5 (accessible mais filtre les signaux négatifs)
  const threshold = 0.5;
...
async function executeAITrade(user, symbol, analysis, exchange, balance) {
  try {
    ...
    // 4. Cooldown — désactivé pour l'IA 100% autonome (elle décide elle-même)
...
// Ancienne connectKrakenWS4h retirée — remplacée par version IA
...
function watchWebSocket() {
  ...
  if (!ws4hOk) {
    watchAlert('WEBSOCKET', 'OHLC 4h WebSocket IA déconnecté — reconnexion');
