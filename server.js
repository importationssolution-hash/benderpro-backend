...
const UserSchema = new mongoose.Schema({
  ...
  xlmWallet:    String, // adresse XLM pour paiement commissions
...
// DÃ‰TECTION DE FIGURES
// SL = bas technique de la figure (support rÃ©el)
// TP = hauteur de la figure projetÃ©e vers le haut
// Ratio R/R minimum 2:1 obligatoire
function detectFigure(closes, volumes, livePrice) {
  ...
  // Hauteur globale de la figure â€” dÃ©termine le TP
  const figureTarget = (high - low) / low;
  if (figureTarget < 0.50) return null; // minimum 50% de hausse projetÃ©e

  // TP dynamique selon la hauteur de la figure (minimum 50%)
  let tpPct;
  if      (figureTarget >= 0.80) tpPct = 0.20; // 80%+  â†’ TP +20%
  else if (figureTarget >= 0.71) tpPct = 0.17; // 71-80% â†’ TP +17%
  else if (figureTarget >= 0.61) tpPct = 0.15; // 61-70% â†’ TP +15%
  else                           tpPct = 0.13; // 50-60% â†’ TP +13%

  // Helper: calcule TP et SL pour une figure
  // SL = sous le bas du support avec buffer 1%
  // TP dynamique selon hauteur de la figure (dÃ©jÃ  calculÃ©)
  function buildLevels(supportLow) {
    const tp = +(price * (1 + tpPct)).toFixed(8);
    const sl = +(price * (1 - SL_PCT)).toFixed(8); // SL fixe -2% du prix d'entrÃ©e
    return { tp, sl };
  }
  ...
  if (n >= 100) {
    ...
    if (cupLow < closes[n-70] * 0.95 && price > resistance && volRatio > 1.8) {
      const lvl = buildLevels(handleLow);
      if (lvl) return { fig: FIGURES[0], ...lvl, figureTarget };
    }
  }

  // â”€â”€ Ã‰PAULE-TÃŠTE-Ã‰PAULE INVERSE â”€â”€
  if (n >= 100) {
    ...
    if (headLow < shoulderLow * 0.97 && price > neckline && volRatio > 1.5) {
      const lvl = buildLevels(headLow);
      if (lvl) return { fig: FIGURES[1], ...lvl, figureTarget };
    }
  }
  ...
  if (n >= 70) {
    ...
    if (Math.abs(bot1 - bot2) / bot1 < 0.02 && price > midTop && volRatio > 1.4) {
      const lvl = buildLevels(Math.min(bot1, bot2));
      if (lvl) return { fig: FIGURES[2], ...lvl, figureTarget };
    }
  }

  // â”€â”€ TRIANGLE ASCENDANT â”€â”€
  if (range < 0.04 && trend10 > 0.01 && price > high * 0.999 && volRatio > 1.6) {
    const triLow = Math.min(...closes.slice(n - 30, n - 1));
    const lvl = buildLevels(triLow);
    if (lvl) return { fig: FIGURES[3], ...lvl, figureTarget };
  }

  // â”€â”€ DRAPEAU HAUSSIER â”€â”€
  if (trend10 > 0.06 && range < 0.025 && price > high * 0.999 && volRatio > 1.8) {
    const flagLow = Math.min(...closes.slice(n - 15, n - 1));
    const lvl = buildLevels(flagLow);
    if (lvl) return { fig: FIGURES[4], ...lvl, figureTarget };
  }

  // â”€â”€ BISEAU BAISSIER (bullish) â”€â”€
  if (range < 0.035 && trend10 < -0.02 && trend10 > -0.05 &&
      price > high * 0.999 && volRatio > 1.7) {
    const biseauLow = Math.min(...closes.slice(n - 20, n - 1));
    const lvl = buildLevels(biseauLow);
    if (lvl) return { fig: FIGURES[5], ...lvl, figureTarget };
  }
...
const livePrices = {};
const lastScannedPrice = {}; // prix au moment du dernier scan par paire

// Confirmation 3 ticks â€” Ã©vite les faux breakouts
// { 'BTC/USD': { count: 2, resistance: 65000, figure: 'Cup & Handle', signal: {...} } }
    ...
    try {
      ...
      if (msg.channel === 'ticker' && msg.data) {
        for (const t of msg.data) {
          if (t.symbol && t.last) {
            // Prix live â€” TP/SL en temps rÃ©el
            livePrices[t.symbol] = t.last;
            const alt = t.symbol.replace('XBT','BTC').replace('BTC','XBT');
            if (alt !== t.symbol) livePrices[alt] = t.last;

            // â”€â”€ CONFIRMATION 3 TICKS â”€â”€
            // Si une cassure est en attente de confirmation pour cette paire
            const bc = breakoutConfirm[t.symbol];
            if (bc) {
              if (t.last > bc.resistance) {
                // Prix toujours au-dessus â†’ incrÃ©menter le compteur
                bc.count++;
                if (bc.count >= 3) {
                  // 3 ticks confirmÃ©s â†’ entrÃ©e immÃ©diate
                  console.log(`[Breakout] ${t.symbol} confirmÃ© ${bc.count} ticks > ${bc.resistance} â†’ ORDRE`);
                  delete breakoutConfirm[t.symbol]; // reset
                  executeTrade(bc.signal).catch(() => {});
                }
              } else {
                // Prix redescendu sous la rÃ©sistance â†’ faux breakout, reset
                console.log(`[Breakout] ${t.symbol} faux breakout â€” reset (prix ${t.last} < rÃ©sistance ${bc.resistance})`);
    ...
    try {
      ...
      if (msg.channel === 'ohlc' && (msg.type === 'snapshot' || msg.type === 'update') && msg.data) {
        for (const c of msg.data) {
          ...
          // Scanner cette paire seulement Ã  la clÃ´ture d'une NOUVELLE bougie
          // â†’ Ã©vite les faux signaux en plein mouvement
          const isNewCandle = !lastC || arr[arr.length-1] !== lastC;
          if (isNewCandle) {
            setImmediate(() => scanSinglePair(sym, '1d'));
          }
        }
      }
    } catch (e) {}
    ...
    try {
      ...
      if (msg.channel === 'ohlc' && (msg.type === 'snapshot' || msg.type === 'update') && msg.data) {
        for (const c of msg.data) {
          ...
          // Scanner seulement Ã  la clÃ´ture d'une nouvelle bougie 4h
          const isNew4h = !lastC || arr[arr.length-1] !== lastC;
          if (isNew4h) {
...
function scanSinglePair(symbol, timeframe = '1d') {
  try {
    ...
    // Prix live tick par tick â€” c'est CE prix qui dÃ©cide de la cassure
    // pas le close de la derniÃ¨re bougie
    const livePrice = livePrices[symbol]; // undefined si pas encore reÃ§u
    if (!livePrice) return; // pas de prix live = on skip, pas d'entrÃ©e sur donnÃ©es stales
    ...
    const rateKey = symbol + '|' + sig.fig.name + '|ratÃ©';
    if (recentSignals.get(rateKey)) return; // signal dÃ©jÃ  ratÃ© (fonds insuf) â€” oubliÃ©
    const lastSigKey = recentSignals.get(sigKey);
    if (lastSigKey && Date.now() - lastSigKey < 60 * 60 * 1000) return; // 1h anti-spam
    ...
    // Calculer la rÃ©sistance (niveau de cassure) pour la confirmation
    // C'est le max des 30 derniÃ¨res bougies = niveau que le prix vient de casser
    const resistance = Math.max(...closes.slice(-30));

    // DÃ©marrer la confirmation 3 ticks au lieu d'entrer directement
    // â†’ Ã©vite les faux breakouts en plein mouvement
    if (!breakoutConfirm[symbol]) {
      breakoutConfirm[symbol] = {
        count:      1,        // premier tick dÃ©jÃ  au-dessus
        resistance: resistance,
        figure:     sig.fig.name,
        signal:     signal,
        startedAt:  Date.now()
      };
      console.log(`[Breakout] ${symbol} Â· ${sig.fig.name} Â· rÃ©sistance ${resistance} Â· confirmation 1/3`);
    }

    // Timeout de sÃ©curitÃ©: si pas confirmÃ© en 30s â†’ annuler
    setTimeout(() => {
      if (breakoutConfirm[symbol] && breakoutConfirm[symbol].figure === sig.fig.name) {
        console.log(`[Breakout] ${symbol} â€” timeout 30s sans confirmation â€” annulÃ©`);
...
async function executeTrade(signal) {
  try {
    ...
    for (const user of users) {
      try {
        // â”€â”€ RÃˆGLE 1: Position dÃ©jÃ  ouverte sur cette paire â†’ skip absolu
        const existingPos = await OpenPosition.findOne({ email: user.email, symbol: signal.symbol });
        if (existingPos) {
          console.log(`[Bot] Position ouverte sur ${signal.symbol} â€” skip`);
          continue;
        }

        // â”€â”€ RÃˆGLE 2: Trade dÃ©jÃ  exÃ©cutÃ© sur cette figure rÃ©cemment
        // MÃªme figure + prix similaire (<20% d'Ã©cart) = mÃªme formation, on ne retrade pas
        const lastTrade = await Trade.findOne(
          { email: user.email, symbol: signal.symbol, figure: signal.figure },
          null,
          { sort: { time: -1 } }
        );
        if (lastTrade) {
          ...
          if (priceDiff < 0.20) {
            console.log(`[Bot] ${signal.symbol} â€” figure ${signal.figure} dÃ©jÃ  tradÃ©e Ã  ${lastTrade.entryPrice} (Ã©cart ${(priceDiff*100).toFixed(1)}% < 20%) â€” attente nouvelle formation`);
            continue;
          }
          console.log(`[Bot] ${signal.symbol} â€” nouvelle formation dÃ©tectÃ©e (Ã©cart ${(priceDiff*100).toFixed(1)}% > 20%) â€” entrÃ©e autorisÃ©e`);
        }

        // â”€â”€ RÃˆGLE 3: Pas de trade dans les derniÃ¨res 4h sur cette paire (toutes figures)
        // Ã‰vite les entrÃ©es multiples rapprochÃ©es sur la mÃªme crypto
        ...
        if (recentTrade) {
          console.log(`[Bot] ${signal.symbol} â€” trade il y a moins de 4h â€” skip`);
          continue;
        }
        ...
        if (usd < amount) {
          console.log(`[Bot] âš  ${signal.symbol} â€” fonds insuffisants (${usd.toFixed(2)}$ dispo / ${amount}$ requis) â€” position oubliÃ©e, next`);
          // Marquer ce signal comme "ratÃ©" dans recentSignals pour ne pas rÃ©essayer
          // Marquer comme ratÃ© pour les deux timeframes
          recentSignals.set(signal.symbol + '|' + signal.figure + '|1d|ratÃ©', Date.now());
          recentSignals.set(signal.symbol + '|' + signal.figure + '|4h|ratÃ©', Date.now());
          continue; // oubli immÃ©diat
        }
  ...
  try {
    ...
    for (const pos of positions) {
      ...
      if (!currentPrice) {
        ...
        if (nPrices === 0) { console.log('[TP/SL] âš  Ticker pas encore connectÃ© â€” livePrices vide'); }
        else { console.log(`[TP/SL] âš  Prix manquant pour ${pos.symbol} (${nPrices} prix en mÃ©moire)`); }
        continue;
      }
      ...
      try {
        ...
        await Trade.findOneAndUpdate(
          { email: pos.email, symbol: pos.symbol, result: 'OPEN' },
          { exitPrice: currentPrice, pnl, result: hitTP ? 'WIN' : 'LOSS', exitReason: hitTP ? `TP +${(pos.tpPct||TP_PCT*100)}% atteint` : `SL touchÃ© (support technique)` },
          { sort: { time: -1 } }
        );
        await OpenPosition.deleteOne({ _id: pos._id });
        ...
        for (const sig of userSignals.slice(0, MAX_CONCURRENT)) {
          ...
          try {
            ...
            if (sig.direction === 'Long' && quoteBalance >= amount) {
              // RÃ¨gle 1: position dÃ©jÃ  ouverte
              ...
              // RÃ¨gle 2: mÃªme figure dÃ©jÃ  tradÃ©e rÃ©cemment
              const lastTrade = await Trade.findOne(
                { email: user.email, symbol: sig.symbol, figure: sig.figure },
                null, { sort: { time: -1 } }
              );
              if (lastTrade) {
                const priceDiff = Math.abs(sig.entryPrice - lastTrade.entryPrice) / lastTrade.entryPrice;
                if (priceDiff < 0.20) { console.log(`[Bot] ${sig.symbol} â€” mÃªme formation â€” skip`); continue; }
              }

              // RÃ¨gle 3: pas de trade dans les 4 derniÃ¨res heures sur cette paire
              const recentTrade = await Trade.findOne({
                email: user.email, symbol: sig.symbol,
                time: { $gte: new Date(Date.now() - 4 * 60 * 60 * 1000) }
              });
              if (recentTrade) { console.log(`[Bot] ${sig.symbol} â€” trade rÃ©cent (<4h) â€” skip`); continue; }
...
// Route pour enregistrer l'email seul (avant la connexion API)
...
// Route positions actives avec prix live intÃ©grÃ©s
app.get('/positions/:email', async (req, res) => {
  try {
    const email = req.params.email;
    const positions = await OpenPosition.find({ email });
    // Enrichir chaque position avec le prix live du ticker WebSocket
    const enriched = positions.map(pos => ({
      ...pos.toObject(),
      currentPrice: livePrices[pos.symbol] || pos.entryPrice,
      pnlPct: livePrices[pos.symbol]
        ? ((livePrices[pos.symbol] - pos.entryPrice) / pos.entryPrice * 100)
        : 0,
      pnlUsd: livePrices[pos.symbol]
        ? ((livePrices[pos.symbol] - pos.entryPrice) / pos.entryPrice) * pos.amount
        : 0,
    }));
...
// Sauvegarder le wallet XLM de l'utilisateur
app.post('/save-xlm', async (req, res) => {
  try {
    const { email, xlmWallet } = req.body;
    if (!email || !xlmWallet) return res.json({ success: false, error: 'DonnÃ©es manquantes' });
    if (!xlmWallet.startsWith('G') || xlmWallet.length < 40)
...
// DÃ©connexion complÃ¨te â€” dÃ©sactive toutes les sessions avec cet email
app.post('/disconnect', async (req, res) => {
  try {
    const { email } = req.body;
    if (!email) return res.json({ success: false, error: 'Email manquant' });
    // DÃ©sactiver le bot pour cet email
    await User.findOneAndUpdate({ email }, { active: false });
    console.log(`[Disconnect] ${email} dÃ©connectÃ© â€” toutes les sessions fermÃ©es`);
    res.json({ success: true, message: 'Bot arrÃªtÃ© â€” toutes les sessions dÃ©connectÃ©es' });
  } catch (e) { res.json({ success: false, error: e.message }); }
});
...
// â”€â”€ VÃ©rification paiements XLM toutes les 24h â”€â”€
async function checkXlmPayments() {
  try {
    ...
    // RÃ©cupÃ©rer les transactions rÃ©centes vers notre wallet via Horizon API
    const horizonUrl = `https://horizon.stellar.org/accounts/${BENDER_XLM}/payments?order=desc&limit=50`;
    const resp = await fetch(horizonUrl);
    const data = await resp.json();
    const payments = data._embedded?.records || [];

    for (const user of users) {
      if (!user.xlmWallet) continue;
      // Chercher un paiement de ce wallet vers le nÃ´tre
      const payment = payments.find(p =>
        p.from === user.xlmWallet &&
        p.asset_type === 'native' && // XLM natif
        p.to === BENDER_XLM
      );
      if (!payment) continue;

      // VÃ©rifier si ce paiement correspond Ã  une facture en attente
      ...
      await Billing.findOneAndUpdate(
        { _id: billing._id },
        { status: 'PAID', paidAt: new Date(), txHash: payment.transaction_hash }
      );
      console.log(`[XLM] Paiement dÃ©tectÃ© pour ${user.email} â€” facture marquÃ©e PAID`);
    }
  } catch (e) {
    console.log('[XLM] Erreur vÃ©rification paiements:', e.message);
  }
}
  ...
  setImmediate(async () => {
    ...
    // Premier check dans 100ms â€” pas d'attente
    setTimeout(() => checkTPSLInstant().catch(console.error), 100);
    setInterval(() => checkTPSLInstant().catch(console.error), 2000);
    console.log(' Suivi TP/SL instantanÃ© actif (toutes les 2 secondes dÃ¨s maintenant)');

      // VÃ©rification paiements XLM toutes les 24h
      setTimeout(() => checkXlmPayments().catch(console.error), 5000);
      setInterval(() => checkXlmPayments().catch(console.error), 24 * 60 * 60 * 1000);
      console.log(' VÃ©rification paiements XLM active (toutes les 24h)');
