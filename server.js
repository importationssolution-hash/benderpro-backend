...
function detectFigure(closes, volumes, livePrice, aiParams) {
  ...
  const volRatio = volumes[n-1] / avg(volumes.slice(-50));
  ...
  function buildLevels(figHigh, figLow) {
    const h = figHigh - figLow;
    const ft = h / figLow;
    if (ft < 0.40) return null;
    const tpPct = ft>=0.80?0.20:ft>=0.60?0.17:ft>=0.50?0.15:0.13;
    return { tp:+(price*(1+tpPct)).toFixed(8), sl:+(price*(1-SL_PCT)).toFixed(8), figureTarget:ft, tpPct };
  }

  // Cup & Handle
  if (n>=100) {
    const cupLow=Math.min(...closes.slice(n-80,n-20));
    const res=Math.max(...closes.slice(n-30,n-1));
    const lv=buildLevels(res,cupLow);
    if(lv&&cupLow<closes[n-70]*0.95&&price>res*0.98&&volRatio>1.8&&trendBull&&rsiOk)
      return {fig:FIGURES[0],...lv};
  }
  // ETE Inverse
  if (n>=100) {
    const head=Math.min(...closes.slice(n-60,n-20));
    const sho=Math.min(...closes.slice(n-80,n-60));
    const neck=Math.max(...closes.slice(n-80,n-2));
    const lv=buildLevels(neck,head);
    if(lv&&head<sho*0.97&&price>neck*0.98&&volRatio>1.5&&rsiOk)
      return {fig:FIGURES[1],...lv};
  }
  // Double Bottom
  if (n>=70) {
    const b1=Math.min(...closes.slice(n-50,n-25));
    const b2=Math.min(...closes.slice(n-25,n-1));
    const mid=Math.max(...closes.slice(n-40,n-10));
    const lv=buildLevels(mid,(b1+b2)/2);
    if(lv&&Math.abs(b1-b2)/b1<0.03&&mid>(b1+b2)/2*1.05&&price>mid*0.98&&volRatio>1.4&&rsiOk)
      return {fig:FIGURES[2],...lv};
  }
  // Triangle Ascendant
  if (n>=40) {
    const r30=Math.max(...closes.slice(n-30,n));
    const s30=Math.min(...closes.slice(n-30,n));
    const lv=buildLevels(r30,s30);
    if(lv&&(r30-s30)/price<0.06&&trend10>0.01&&price>r30*0.995&&volRatio>1.6&&trendBull&&rsiOk)
      return {fig:FIGURES[3],...lv};
  }
  // Drapeau Haussier
  if (n>=30) {
    const ms=closes[n-25];
    const mt=Math.max(...closes.slice(n-20,n-8));
    const fl=Math.min(...closes.slice(n-8,n-1));
    const lv=buildLevels(mt+(mt-fl),fl);
    if(lv&&(mt-ms)/ms>0.15&&(mt-fl)/mt<0.12&&fl>ms*1.05&&price>mt*0.98&&volRatio>1.8&&trend10>0.05)
      return {fig:FIGURES[4],...lv};
  }
  // Biseau Baissier
  if (n>=30) {
    const bh=Math.max(...closes.slice(n-25,n-1));
    const bl=Math.min(...closes.slice(n-25,n-1));
    const rr=calcRSI(closes.slice(-20));
    const ro=calcRSI(closes.slice(-40,-20));
    const lv=buildLevels(bh,bl);
    if(lv&&(bh-bl)/bh<0.08&&trend10<-0.01&&trend10>-0.10&&rr>ro&&price>bh*0.995&&volRatio>1.7)
      return {fig:FIGURES[5],...lv};
  }
  return null;
}
    ...
    if (msg.channel==='ohlc' && (msg.type==='snapshot'||msg.type==='update') && msg.data) {
      for (const c of msg.data) {
        ...
        if (!isNewCandle) {
          ...
          // Volume quotidien (Daily uniquement)
          if (store === krakenCandles && c.volume && c.close) {
            const rv = arr.slice(-30).map(b=>b.c*b.v).filter(v=>v>0);
            if (rv.length>0) pairDailyVolume[sym] = rv.reduce((a,b)=>a+b,0)/rv.length;
          }
...
function connectKrakenTicker(pairs) {
  ...
  wsTicker.on('error',(e)=>console.log('[Ticker]',e.message));
}
    ...
    await Promise.all(batch.map(async (symbol) => {
      try {
        ...
        if (d.length >= 10) {
          ...
          const rv=d.slice(-30).map(b=>b.c*b.v).filter(v=>v>0);
...
function scanSinglePair(symbol, timeframe='1d', exchangeId='kraken', candleStoreOverride=null) {
  try {
    ...
    if (!livePrice) return;
    if (livePrice < 0.001) return; // prix trop bas
    // Calculer volume si pas dans le cache
...
// Route chatbot builder — proxy vers API Anthropic
  ...
  try {
    ...
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 1000,
        system: system || 'Tu es un assistant de trading.',
    ...
    res.json({ success: false, error: e.message, reply: '⚠ Service temporairement indisponible.' });
  }
});
...
app.listen(PORT, () => {
  ...
  setImmediate(async () => {
    ...
    if (krakenPairsList.length > 0) {
      ...
      connectKrakenWS4h(krakenPairsList);   // 4H     // Daily 1D
      connectKrakenWS1h(krakenPairsList);   // 1H
    ...
    // Diagnostic 5min
    setInterval(() => {
      const qv=Object.keys(pairDailyVolume).filter(k=>pairDailyVolume[k]>=MIN_DAILY_VOLUME_USD).length;
      const qc=Object.keys(krakenCandles).filter(k=>(krakenCandles[k]||[]).length>=100).length;
      const nb=Object.keys(breakoutConfirm).length;
      console.log(`[Diag] ${qc} paires·${qv} qualifiées·${nb} cassures en attente·${Object.keys(livePrices).length} prix`);
    }, 5*60*1000);
