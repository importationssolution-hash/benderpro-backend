...
function generateAIStrategy(version) {
  return {
    ...
    cooldown:       0, // IA 100% autonome â€” pas de cooldown forcÃ©
  };
}
...
function aiScore(closes, volumes, strategy, livePrice) {
  if (closes.length < 50) return { score: 0, shouldBuy: false };
  ...
  // Seuil: score doit Ãªtre positif (l'IA cherche des opportunitÃ©s)
  // Threshold = 0.5 (accessible mais filtre les signaux nÃ©gatifs)
  const threshold = 0.5;
...
async function executeAITrade(user, symbol, analysis, exchange, balance) {
  try {
    ...
    // 4. Cooldown â€” dÃ©sactivÃ© pour l'IA 100% autonome (elle dÃ©cide elle-mÃªme)
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
const krakenCandles    = {};
const krakenCandles1h  = {}; // 1H IA
const krakenCandles4h  = {}; // 4H IA
let krakenPairsList    = [];
...
// WebSocket 1H — IA — 500 paires en batches progressifs
function connectKrakenWS1h(pairs) {
  if (!pairs||!pairs.length) return;
  if (ws1h) { try{ws1h.terminate();}catch(e){} }
  ws1h = new WebSocket('wss://ws.kraken.com/v2');
  ws1h.on('open', () => {
    console.log(`[WS-1H IA] ${pairs.length} paires — abonnement progressif`);
    // Envoyer 25 paires toutes les 200ms pour ne pas saturer Kraken
    let i = 0;
    const sendBatch = () => {
      if (i >= pairs.length || !ws1h || ws1h.readyState !== 1) return;
      ws1h.send(JSON.stringify({method:'subscribe',params:{channel:'ohlc',symbol:pairs.slice(i,i+25),interval:60}}));
      i += 25;
      if (i < pairs.length) setTimeout(sendBatch, 200);
      else console.log(`[WS-1H] ${pairs.length} paires abonnées`);
    };
    sendBatch();
    // Ping géré par le ping global (setInterval 20s au boot)
  });
    ...
    try{
      ...
      if(msg.channel==='ohlc'&&(msg.type==='snapshot'||msg.type==='update')&&msg.data){
        for(const c of msg.data){
          ...
          if(last&&Math.abs(last.c-c.close)/(c.close||1)<0.5){last.c=c.close;last.v=c.volume;}
          else{arr.push({c:c.close,v:c.volume});if(arr.length>300)arr.shift();}
        }
      }
    }catch(e){}
  });
  ws1h.on('close',()=>{if(ws1h._hb)clearInterval(ws1h._hb);setTimeout(()=>connectKrakenWS1h(krakenPairsList),15000);});
  ws1h.on('error',(e)=>console.log('[WS-1H]',e.message));
}

// WebSocket 4H — IA — 500 paires en batches progressifs
function connectKrakenWS4h(pairs) {
  if (!pairs||!pairs.length) return;
  if (ws4h) { try{ws4h.terminate();}catch(e){} }
  ws4h = new WebSocket('wss://ws.kraken.com/v2');
  ws4h.on('open', () => {
    console.log(`[WS-4H IA] ${pairs.length} paires — abonnement progressif`);
    let i = 0;
    const sendBatch = () => {
      if (i >= pairs.length || !ws4h || ws4h.readyState !== 1) return;
      ws4h.send(JSON.stringify({method:'subscribe',params:{channel:'ohlc',symbol:pairs.slice(i,i+25),interval:240}}));
      i += 25;
      if (i < pairs.length) setTimeout(sendBatch, 200);
      else console.log(`[WS-4H] ${pairs.length} paires abonnées`);
    };
    sendBatch();
    // Ping géré par le ping global (setInterval 20s au boot)
  });
  ...
  ws4h.on('close',()=>{if(ws4h._hb)clearInterval(ws4h._hb);setTimeout(()=>connectKrakenWS4h(krakenPairsList),15000);});
...
async function preloadHistoricalCandles(pairs) {
  ...
  for (let i=0; i<pairs.length; i+=BATCH) {
    ...
    await Promise.all(batch.map(async (symbol) => {
      ...
      try {
        const h1 = await exchange.fetchOHLCV(symbol,'1h',undefined,200);
        if (h1 && h1.length>=10) krakenCandles1h[symbol] = h1.map(c=>({c:c[4],v:c[6]||c[5]}));
      } catch(e) {}
      try {
        const h4 = await exchange.fetchOHLCV(symbol,'4h',undefined,200);
...
function watchWebSocket() {
  ...
  if(!wsOk1h){watchAlert('WEBSOCKET','1H IA KO');   connectKrakenWS1h(krakenPairsList);}
  if(!wsOk4h){watchAlert('WEBSOCKET','4H IA KO');   connectKrakenWS4h(krakenPairsList);}
