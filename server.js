...
const AI_VERSION_KEY    = 'aiVersion'; // clÃ© en DB pour la version IA active
...
function detectFigure(closes, volumes, livePrice) {
  ...
  // Figure valide si hauteur >= 40% â€” pas de limite de gain max!
  // Si la figure montre 1000% de potentiel â†’ on prend 1000%
  if (figureTarget < 0.40) return null; // trop faible â€” moins de 40% de hausse potentielle

  // TP = 25% de la hauteur de la figure
  // Exemple: figure de 66% â†’ TP = 66% Ã— 25% = +16.5%
  const tpPct = +(figureTarget * 0.25).toFixed(4);
  function buildLevels() {
    // TP = 25% de la hauteur de la figure au-dessus du prix d'entrÃ©e
    const tp = +(price * (1 + tpPct)).toFixed(8);

    // SL = bas de la figure - 0.5% buffer
    // low = plus bas des 500 derniÃ¨res bougies (bas de la figure)
    const sl = +(low * 0.995).toFixed(8);

    // SL en % par rapport au prix d'entrÃ©e (pour affichage)
    const slPctDynamic = +((price - sl) / price * 100).toFixed(2);

    return { tp, sl, slPct: slPctDynamic };
  }
  // DÃ‰TECTION EN FORMATION â€” prix peut Ãªtre sous la rÃ©sistance
  // C'est scanSinglePair qui gÃ¨re l'attente et l'entrÃ©e Ã  la cassure

  if (n >= 50) {
    // CUP & HANDLE: creux profond suivi d'un handle compact prÃ¨s de la rÃ©sistance
    const cupLow    = Math.min(...closes.slice(n-60,n-20));
    const resistance = Math.max(...closes.slice(n-30,n-1));
    const nearRes   = price >= resistance * 0.90 && price <= resistance * 1.05;
    if (cupLow < closes[n-70]*0.95 && nearRes && volRatio > 1.0)
      return { fig:FIGURES[0], ...buildLevels(), figureTarget, tpPct, resistance };
  }
  if (n >= 50) {
    // ETE INVERSE: tÃªte plus basse + prix remonte vers la ligne de cou
    ...
    const nearNeck   = price >= neckline * 0.90 && price <= neckline * 1.05;
    if (headLow < shoulderLow*0.97 && nearNeck && volRatio > 1.0)
      return { fig:FIGURES[1], ...buildLevels(), figureTarget, tpPct, resistance: neckline };
  }
  if (n >= 50) {
    // DOUBLE BOTTOM: deux creux similaires + prix remonte vers le sommet
    ...
    const nearMid = price >= midTop * 0.90 && price <= midTop * 1.05;
    if (Math.abs(bot1-bot2)/bot1 < 0.03 && nearMid && volRatio > 1.0)
      return { fig:FIGURES[2], ...buildLevels(), figureTarget, tpPct, resistance: midTop };
  }
  // TRIANGLE ASCENDANT: range serrÃ© + tendance + prix proche du plus haut
  if (range < 0.06 && trend10 > 0.01 && price >= high*0.92 && volRatio > 1.0)
    return { fig:FIGURES[3], ...buildLevels(), figureTarget, tpPct, resistance: high };
  // DRAPEAU HAUSSIER: forte montÃ©e suivie d'une consolidation
  if (trend10 > 0.04 && range < 0.04 && price >= high*0.90 && volRatio > 1.0)
    return { fig:FIGURES[4], ...buildLevels(), figureTarget, tpPct, resistance: high };
  // BISEAU BAISSIER: lÃ©gÃ¨re baisse puis remontÃ©e
  if (range < 0.05 && trend10 < -0.01 && trend10 > -0.08 && price >= high*0.90 && volRatio > 1.0)
    return { fig:FIGURES[5], ...buildLevels(), figureTarget, tpPct, resistance: high };
  return null;
}
...
// Choisir les timeframes â€” l'IA explore librement 1h, 4h, 1d
function pickAITimeframes() {
  const all = ['1h', '4h', '1d'];
  const nChoices = 1 + Math.floor(Math.random() * 3); // 1 Ã  3 timeframes
  const shuffled = [...all].sort(() => Math.random() - 0.5);
  return shuffled.slice(0, nChoices);
}
...
function generateAIStrategy(version) {
  return {
    ...
    volumeThresh:   1.2 + Math.random() * 1.5,              // 1.2x Ã  2.7x la moyenne
    trendPeriod:    Math.floor(10 + Math.random() * 40),     // 10-50 bougies
    tpMultiplier:   0.05 + Math.random() * 0.15,            // TP 5% Ã  20%
    minConfidence:  50 + Math.floor(Math.random() * 30),     // 50% Ã  80%
    timeframes:     pickAITimeframes(mem.bestTimeframes),
    preferredHour:  Math.floor(Math.random() * 24),          // heure prÃ©fÃ©rÃ©e pour trader
    ...
    maxPositions:   Math.floor(2 + Math.random() * 8),       // 2 Ã  10 positions max
  ...
  try {
    ...
    for (const user of aiUsers) {
      ...
      if (dayPnl < AI_MIN_GREEN) {
        // IA dans le rouge â†’ la remplacer
        const oldVersion = mem.version || 1;
        const newVersion = oldVersion + 1;

        console.log(`[IA] ${user.email} â€” Version IA_${oldVersion} dans le rouge (P&L: ${dayPnl.toFixed(4)}$) â†’ Remplacement par IA_${newVersion}`);

        // Archiver l'ancienne version
        await AIVersion.findOneAndUpdate(
          { email: user.email, version: oldVersion },
          { status: 'replaced', retiredAt: new Date(), pnlDay: dayPnl, reason: `P&L journalier nÃ©gatif: ${dayPnl.toFixed(4)}$` }
        );

        // CrÃ©er la nouvelle stratÃ©gie
        ...
        console.log(`[IA] ${user.email} â€” IA_${newVersion} activÃ©e avec nouvelle stratÃ©gie`);
      } else {
        console.log(`[IA] ${user.email} â€” IA_${mem.version} dans le vert (P&L: +${dayPnl.toFixed(4)}$) â†’ Continue demain`);
...
async function initAIMemory(email) {
  ...
  if (activeVersion) {
    ...
    console.log(`[IA] ${email} â€” IA_1 crÃ©Ã©e avec stratÃ©gie initiale`);
  }
  ...
  try {
    ...
    for (const user of aiUsers) {
      try {
        ...
        // L'IA scanne les timeframes choisis par SA stratÃ©gie (1h, 4h, 1d ou combinaison)
        const tfMap = { '1d': krakenCandles, '4h': krakenCandles4h, '1h': krakenCandles1h };
        const activeTfs = (strategy.timeframes && strategy.timeframes.length > 0) ? strategy.timeframes : ['1d'];

        let bestSignal = null;
        let bestScore = 0;

        for (const tf of activeTfs) {
          const candleStore = tfMap[tf] || krakenCandles;
          const pairs = Object.keys(candleStore).filter(sym => {
            const candles = candleStore[sym];
            return candles && candles.length >= 50;
          });
        ...
        // Entrer sur le meilleur signal trouvÃ© (parmi tous les timeframes scannÃ©s)
        if (bestSignal) {
          console.log(`[IA_${mem.version}] Meilleur signal: ${bestSignal.symbol} Â· TF:${bestSignal.timeframe} Â· Score:${bestSignal.score.toFixed(2)}`);
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
const krakenCandles    = {}; // Daily â€” utilisÃ© par bot Classique ET IA
const krakenCandles1h  = {}; // 1H â€” utilisÃ© par l'IA uniquement
const krakenCandles4h  = {}; // 4H â€” utilisÃ© par l'IA uniquement
let krakenPairsList    = [];
    ...
    try {
      ...
      if (msg.channel==='ohlc' && (msg.type==='snapshot'||msg.type==='update') && msg.data) {
        for (const c of msg.data) {
          ...
          else { arr.push({c:c.close,v:c.volume}); if (arr.length>500) arr.shift(); }
...
function connectKrakenWS4h(pairs) {
  ...
  ws4h.on('open', () => {
    console.log(`[WS-4h] ConnectÃ© â€” ${pairs.length} paires`);
    for (let i=0; i<pairs.length; i+=50)
    ...
    try {
      ...
      if (msg.channel==='ohlc' && (msg.type==='snapshot'||msg.type==='update') && msg.data) {
        for (const c of msg.data) {
          ...
          // Scan 4h dÃ©sactivÃ© â€” Daily uniquement
        }
      }
    } catch(e) {}
  });
  ws4h.on('close', () => { /* 4h dÃ©sactivÃ© */ });
...
function connectKrakenWS1h(pairs) {
  ...
  ws1h.on('open', () => {
    console.log(`[WS-1h IA] ConnectÃ© â€” ${pairs.length} paires`);
    for (let i=0; i<pairs.length; i+=50)
      ws1h.send(JSON.stringify({ method:'subscribe', params:{ channel:'ohlc', symbol:pairs.slice(i,i+50), interval:60 } }));
  });
  ws1h.on('message', (raw) => {
    try {
      const msg = JSON.parse(raw);
      if (msg.channel==='ohlc' && (msg.type==='snapshot'||msg.type==='update') && msg.data) {
        for (const c of msg.data) {
          const sym = c.symbol;
          const arr = krakenCandles1h[sym] || (krakenCandles1h[sym]=[]);
          const lastC = arr[arr.length-1];
          if (lastC && Math.abs(lastC.c-c.close)/(c.close||1) < 0.5) { lastC.c=c.close; lastC.v=c.volume; }
          else { arr.push({c:c.close,v:c.volume}); if (arr.length>500) arr.shift(); }
        }
  ...
  ws1h.on('error', (err) => { console.log('[WS-1h IA] Erreur:', err.message); });
}
...
function connectKrakenWS4h(pairs) {
  ...
  ws4h = new WebSocket('wss://ws.kraken.com/v2');
  ws4h.on('open', () => {
    console.log(`[WS-4h IA] ConnectÃ© â€” ${pairs.length} paires`);
    for (let i=0; i<pairs.length; i+=50)
      ws4h.send(JSON.stringify({ method:'subscribe', params:{ channel:'ohlc', symbol:pairs.slice(i,i+50), interval:240 } }));
  });
  ws4h.on('message', (raw) => {
  ...
  ws4h.on('error', (err) => { console.log('[WS-4h IA] Erreur:', err.message); });
}
...
async function preloadHistoricalCandles(pairs) {
  ...
  for (let i=0; i<pairs.length; i+=BATCH) {
    ...
    await Promise.all(batch.map(async (symbol) => {
      try {
        const d = await exchange.fetchOHLCV(symbol,'1d',undefined,500);
        if (d && d.length>=10) krakenCandles[symbol] = d.map(c=>({c:c[4],v:c[6]||c[5]}));
      } catch(e) {}
      try {
        const h1 = await exchange.fetchOHLCV(symbol,'1h',undefined,500);
        if (h1 && h1.length>=10) krakenCandles1h[symbol] = h1.map(c=>({c:c[4],v:c[6]||c[5]}));
      } catch(e) {}
      try {
        const h4 = await exchange.fetchOHLCV(symbol,'4h',undefined,500);
...
function scanSinglePair(symbol, timeframe='1d', exchangeId='kraken') {
  try {
    ...
    // RÃ©sistance retournÃ©e par la figure ou fallback
    const resistance = sig.resistance || Math.max(...closes.slice(-30));
    const prixVsResistance = (livePrice - resistance) / resistance;

    // Trop tard â€” prix dÃ©jÃ  +4% au-dessus de la rÃ©sistance
    if (prixVsResistance > 0.04) {
      console.log(`[Figure] ${symbol} Â· ${sig.fig.name} Â· Trop tard (+${(prixVsResistance*100).toFixed(1)}%) â€” skip`);
      return;
    }

    // Cassure fraÃ®che (0% Ã  +4%) â†’ ORDRE IMMÃ‰DIAT
    if (prixVsResistance >= 0) {
      console.log(`[Figure] ${symbol} Â· ${sig.fig.name} Â· CASSURE +${(prixVsResistance*100).toFixed(1)}% â†’ ORDRE IMMÃ‰DIAT`);
      executeTrade(signal).catch(() => {});
      return;
    }

    // Prix sous la rÃ©sistance â†’ en attente de cassure
    if (!breakoutConfirm[symbol] || breakoutConfirm[symbol].figure !== sig.fig.name) {
      breakoutConfirm[symbol] = { resistance, figure: sig.fig.name, signal, startedAt: Date.now() };
      console.log(`[Figure] ${symbol} Â· ${sig.fig.name} Â· Attente cassure ${resistance.toFixed(4)} Â· Prix: ${livePrice.toFixed(4)} (${(prixVsResistance*100).toFixed(1)}%)`);
    }
    // Expiration 7 jours
    setTimeout(() => {
      if (breakoutConfirm[symbol] && breakoutConfirm[symbol].figure === sig.fig.name)
...
function scanKrakenFromMemory() {
  ...
  for (const symbol of krakenPairsList) {
    ...
    results.push({
      ...
      resistance:sig.resistance||price,
      tpPct:+(sig.tpPct*100).toFixed(1),slPct:+(SL_PCT*100).toFixed(1),
      figureTarget:+(sig.figureTarget*100).toFixed(1),
      volumeRatio:volRatio.toFixed(2),tradeAmount:TRADE_AMOUNT,
  ...
  try {
    ...
    for (let i=0;i<symbols.length;i+=BATCH) {
      ...
      const batchResults=await Promise.all(batch.map(async(symbol)=>{
        try {
          const ohlcv=await exchange.fetchOHLCV(symbol,'1d',undefined,500);
    ...
    for (const user of users.filter(u=>u.botMode==='classic'||!u.botMode)) {
      ...
      try {
        ...
        for (const sig of userSignals.slice(0,MAX_CONCURRENT)) {
          // VÃ©rifier les fonds pour CE signal prÃ©cis â€” si insuffisant, signal ignorÃ© (plus favorable)
          const freshBalance = await exchange.fetchBalance().catch(()=>balance);
          const freshAvailable = freshBalance[currency]?.free || 0;
          if (freshAvailable < amount) {
            console.log(`[Classic] ${sig.symbol} Â· Fonds insuffisants (${freshAvailable.toFixed(2)}${currency} / ${amount}${currency}) â€” signal ignorÃ©, plus favorable`);
            continue; // ignore CE signal seulement, passe au suivant
          }
          ...
          try {
            ...
            if (recentTrade) continue;
            // VÃ©rifier que le prix actuel est dans la fenÃªtre de cassure (0% Ã  +4%)
            const liveP = livePrices[sig.symbol] || sig.entryPrice;
            const sigRes = sig.resistance || sig.entryPrice;
            const pctAboveRes = (liveP - sigRes) / sigRes;
            if (pctAboveRes > 0.04) {
              console.log(`[Classic] ${sig.symbol} Â· Trop tard (+${(pctAboveRes*100).toFixed(1)}% > +4%) â€” skip`);
              continue;
            }
            if (pctAboveRes < 0) {
              console.log(`[Classic] ${sig.symbol} Â· Pas encore cassÃ© (${(pctAboveRes*100).toFixed(1)}%) â€” skip`);
              continue;
            }
            const qty=amount/liveP; // utiliser le prix live pas le prix du signal
...
function watchWebSocket() {
  ...
  if (!ws4hOk) {
    watchAlert('WEBSOCKET', 'OHLC 4h WebSocket dÃ©connectÃ© â€” reconnexion');
    // connectKrakenWS4h dÃ©sactivÃ© â€” Daily uniquement
  }
...
app.listen(PORT, () => {
  ...
  setImmediate(async () => {
    ...
    if (krakenPairsList.length > 0) {
      connectKrakenTicker(krakenPairsList);
      connectKrakenWS(krakenPairsList);     // Daily â€” Bot Classique ET IA
      connectKrakenWS1h(krakenPairsList);   // 1H â€” IA uniquement
      connectKrakenWS4h(krakenPairsList);   // 4H â€” IA uniquement
      console.log(' Multi-timeframe actif pour IA: 1h, 4h, 1d');
