  ...
  for (let i=0; i<pairs.length; i+=BATCH) {
    ...
    await Promise.all(batch.map(async (symbol) => {
      try {
        ...
        if (d && d.length>=10) krakenCandles[symbol] = d.map(c=>({c:c[4],v:c[6]||c[5],ts:c[0]}));
      } catch(e) {}
      try {
        const h1 = await exchange.fetchOHLCV(symbol,'1h',undefined,300);
        if (h1 && h1.length>=10) krakenCandles1h[symbol] = h1.map(c=>({c:c[4],v:c[6]||c[5],ts:c[0]}));
      } catch(e) {}
      try {
        const h4 = await exchange.fetchOHLCV(symbol,'4h',undefined,300);
        if (h4 && h4.length>=10) krakenCandles4h[symbol] = h4.map(c=>({c:c[4],v:c[6]||c[5],ts:c[0]}));
      } catch(e) {}
    }));
