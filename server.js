...
// CONFIG MODE IA
const AI_MAX_TRADE      = 5;    // Max 5$ par trade (jamais nÃ©gociable)
const AI_MAX_LOSS_DAY   = 0.10; // Max -10% du capital en une journÃ©e
const AI_MIN_RETURN     = 0.10; // Doit faire +10% de rendement pour survivre
const AI_VERSION_KEY    = 'aiVersion';
...
// Schema IA â€” historique des versions avec mÃ©moire Ã©volutive
const AIVersionSchema = new mongoose.Schema({
  email:      String,
  version:    Number,          // numÃ©ro de version IA
  date:       Date,            // date de dÃ©but
  pnlDay:     Number,          // P&L du jour (en %)
  pnlUsd:     Number,          // P&L du jour (en $)
  trades:     Number,          // nombre de trades aujourd'hui
  wins:       Number,          // trades gagnants
  losses:     Number,          // trades perdants
  status:     String,          // 'active', 'survived', 'dead'
  strategy:   Object,          // paramÃ¨tres de la stratÃ©gie IA
  memory:     Object,          // mÃ©moire hÃ©ritÃ©e des gÃ©nÃ©rations prÃ©cÃ©dentes
  retiredAt:  Date,
  reason:     String,          // raison de la mort
  // Apprentissage
  bestCryptos:   [String],     // cryptos les plus profitables
  worstCryptos:  [String],     // cryptos Ã  Ã©viter
  bestHours:     [Number],     // heures les plus profitables
  bestIndicators: Object,      // indicateurs qui ont bien prÃ©dit
});
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
const FIGURES = [
  // slPct = SL dynamique basÃ© sur la structure de la figure
  // Cup & Handle: SL sous le handle (bas du handle ~3-5% sous la rÃ©sistance)
  { name:'Cup & Handle',     code:'C&H',   dir:'Long', wr:0.84, slPct:0.04 },
  // ETE Inverse: SL sous l'Ã©paule droite (~3% sous la ligne de cou)
  { name:'ETE Inverse',      code:'ETEi',  dir:'Long', wr:0.81, slPct:0.03 },
  // Double Bottom: SL sous le 2Ã¨me creux (~2% sous le bas)
  { name:'Double Bottom',    code:'2Bot',  dir:'Long', wr:0.76, slPct:0.025 },
  // Triangle Ascendant: SL sous la derniÃ¨re ligne de support (~2.5%)
  { name:'Triangle Asc.',    code:'TriA',  dir:'Long', wr:0.74, slPct:0.025 },
  // Drapeau Haussier: SL sous le bas du drapeau (~2%)
  { name:'Drapeau Haussier', code:'DrapH', dir:'Long', wr:0.76, slPct:0.02 },
  // Biseau Baissier: SL sous le dernier point bas du biseau (~3%)
  { name:'Biseau Baissier',  code:'BisB',  dir:'Long', wr:0.73, slPct:0.03 },
];
...
function detectFigure(closes, volumes, livePrice) {
  ...
  function buildLevels(fig) {
    // SL dynamique: basÃ© sur la structure de la figure
    const dynamicSlPct = fig ? fig.slPct : SL_PCT;
    // SL = prix d'entrÃ©e - (hauteur de la figure * slPct)
    // Plus la figure est haute, plus le SL est Ã©loignÃ© (structure respectÃ©e)
    const structuralSL = price * (1 - dynamicSlPct);
    // SL ne peut pas Ãªtre plus bas que -8% (protection maximale)
    const sl = +Math.max(structuralSL, price * 0.92).toFixed(8);
    const tp = +(price * (1 + tpPct)).toFixed(8);
    return { tp, sl, slPct: +(dynamicSlPct * 100).toFixed(2) };
  }
  if (n >= 100) {
    ...
    if (cupLow < closes[n-70]*0.95 && price > resistance && volRatio > 1.8)
      return { fig:FIGURES[0], ...buildLevels(FIGURES[0]), figureTarget, tpPct };
  }
  if (n >= 100) {
    ...
    if (headLow < shoulderLow*0.97 && price > neckline && volRatio > 1.5)
      return { fig:FIGURES[1], ...buildLevels(FIGURES[1]), figureTarget, tpPct };
  }
  if (n >= 70) {
    ...
    if (Math.abs(bot1-bot2)/bot1 < 0.02 && price > midTop && volRatio > 1.4)
      return { fig:FIGURES[2], ...buildLevels(FIGURES[2]), figureTarget, tpPct };
  }
  if (range < 0.04 && trend10 > 0.01 && price > high*0.999 && volRatio > 1.6)
    return { fig:FIGURES[3], ...buildLevels(FIGURES[3]), figureTarget, tpPct };
  if (trend10 > 0.06 && range < 0.025 && price > high*0.999 && volRatio > 1.8)
    return { fig:FIGURES[4], ...buildLevels(FIGURES[4]), figureTarget, tpPct };
  if (range < 0.035 && trend10 < -0.02 && trend10 > -0.05 && price > high*0.999 && volRatio > 1.7)
    return { fig:FIGURES[5], ...buildLevels(FIGURES[5]), figureTarget, tpPct };
  return null;
}
...
// GÃ©nÃ©rer une stratÃ©gie IA Ã©volutive â€” hÃ©rite de la mÃ©moire des gÃ©nÃ©rations prÃ©cÃ©dentes
function generateAIStrategy(version, ancestorMemory) {
  const mem = ancestorMemory || {};

  // Base alÃ©atoire
  let strategy = {
    version,
    rsiOversold:    Math.floor(20 + Math.random() * 20),
    rsiOverbought:  Math.floor(60 + Math.random() * 20),
    volumeThresh:   1.2 + Math.random() * 1.5,
    trendPeriod:    Math.floor(10 + Math.random() * 40),
    tpMultiplier:   0.05 + Math.random() * 0.15,
    minConfidence:  50 + Math.floor(Math.random() * 30),
    timeframes:     ['1d'],
    momentumWeight: Math.random(),
    volumeWeight:   Math.random(),
    trendWeight:    Math.random(),
    maxPositions:   Math.floor(2 + Math.random() * 8),
    cooldown:       Math.floor(1 + Math.random() * 6),
    // MÃ©moire hÃ©ritÃ©e
    avoidCryptos:   mem.worstCryptos  || [],  // cryptos Ã  Ã©viter (hÃ©ritÃ©es)
    preferCryptos:  mem.bestCryptos   || [],  // cryptos prÃ©fÃ©rÃ©es (hÃ©ritÃ©es)
    preferHours:    mem.bestHours     || [],  // heures prÃ©fÃ©rÃ©es (hÃ©ritÃ©es)
    generation:     version,
  };

  // Appliquer les leÃ§ons des gÃ©nÃ©rations prÃ©cÃ©dentes
  if (mem.bestIndicators) {
    const bi = mem.bestIndicators;

    // LeÃ§on RSI â€” si beaucoup de pertes, entrer plus sÃ©lectif
    if (bi.avgRsiWin !== undefined) {
      strategy.rsiOversold = Math.max(15, Math.min(35,
        Math.floor(bi.avgRsiWin * 0.85 + Math.random() * 8 - 4)
      ));
    }

    // LeÃ§on Volume â€” si le volume Ã©tait insuffisant sur les pertes
    if (bi.raiseVolThreshold) {
      strategy.volumeThresh = Math.max(1.5, (bi.avgVolWin || 1.8) * (0.95 + Math.random() * 0.15));
    } else if (bi.avgVolWin) {
      strategy.volumeThresh = Math.max(1.1, bi.avgVolWin * (0.85 + Math.random() * 0.3));
    }

    // LeÃ§on TP â€” calibrer vers ce qui a vraiment marchÃ©
    if (bi.avgTpWin) {
      strategy.tpMultiplier = Math.max(0.05, Math.min(0.25,
        bi.avgTpWin * (0.85 + Math.random() * 0.3)
      ));
    }

    // Poids des indicateurs selon leur prÃ©cision historique
    if (bi.momentumAccuracy > 0.5) strategy.momentumWeight = Math.min(1, bi.momentumAccuracy + Math.random() * 0.3 - 0.1);
    if (bi.volumeAccuracy   > 0.5) strategy.volumeWeight   = Math.min(1, bi.volumeAccuracy   + Math.random() * 0.3 - 0.1);
    if (bi.trendAccuracy    > 0.5) strategy.trendWeight    = Math.min(1, bi.trendAccuracy    + Math.random() * 0.3 - 0.1);
  }

  // Heures difficiles globales â†’ les Ã©viter (mais pas par crypto)
  if (mem.badHoursGlobal && mem.badHoursGlobal.length > 0) {
    strategy.avoidHours = mem.badHoursGlobal;
  }

  // LeÃ§ons par crypto â€” ajustements fins, pas de blacklist
  if (mem.cryptoLessons) {
    strategy.cryptoLessons = mem.cryptoLessons;
  }

  console.log(`[IA] StratÃ©gie v${version} gÃ©nÃ©rÃ©e${mem.worstCryptos ? ' (avec mÃ©moire hÃ©ritÃ©e)' : ' (premiÃ¨re gÃ©nÃ©ration)'}`);
  return strategy;
}

// Extraire les LEÃ‡ONS d'une gÃ©nÃ©ration IA
// Pas de blacklist â€” comprendre POURQUOI Ã§a a marchÃ© ou Ã©chouÃ©
async function extractMemory(email, deadVersion) {
  try {
    const dayStart = new Date(deadVersion.date);
    dayStart.setHours(0,0,0,0);
    const dayEnd = new Date(dayStart); dayEnd.setDate(dayEnd.getDate() + 1);

    const trades = await Trade.find({ email, botMode:'ai', time:{ $gte:dayStart, $lt:dayEnd } });
    if (trades.length === 0) return {};

    const wins   = trades.filter(t => t.result === 'WIN');
    const losses = trades.filter(t => t.result === 'LOSS');

    // â”€â”€ ANALYSE DES CAUSES D'ERREUR â”€â”€
    // Pour chaque perte, on analyse le contexte â€” pas la crypto
    const lossPatterns = {};
    losses.forEach(t => {
      const hour   = new Date(t.time).getHours();
      const pnlPct = t.entryPrice ? ((t.exitPrice - t.entryPrice) / t.entryPrice * 100) : 0;
      // Contexte de la perte
      if (!lossPatterns[t.symbol]) lossPatterns[t.symbol] = { losses:0, wins:0, totalPnl:0, hours:[], pnlPcts:[] };
      lossPatterns[t.symbol].losses++;
      lossPatterns[t.symbol].totalPnl += (t.pnl||0);
      lossPatterns[t.symbol].hours.push(hour);
      lossPatterns[t.symbol].pnlPcts.push(pnlPct);
    });
    wins.forEach(t => {
      if (!lossPatterns[t.symbol]) lossPatterns[t.symbol] = { losses:0, wins:0, totalPnl:0, hours:[], pnlPcts:[] };
      lossPatterns[t.symbol].wins++;
      lossPatterns[t.symbol].totalPnl += (t.pnl||0);
    });

    // LeÃ§ons par crypto â€” pas de blacklist, mais des ajustements
    const cryptoLessons = {};
    Object.entries(lossPatterns).forEach(([sym, data]) => {
      const total    = data.wins + data.losses;
      const winRate  = data.wins / Math.max(total, 1);
      const avgLoss  = data.losses > 0 ? data.pnlPcts.reduce((a,b)=>a+b,0)/data.pnlPcts.length : 0;
      // Heures oÃ¹ cette crypto a perdu
      const badHours = [...new Set(data.hours)];

      cryptoLessons[sym] = {
        winRate,
        totalPnl: data.totalPnl,
        // LeÃ§on: si winRate < 40% sur cette crypto â†’ exiger plus de volume la prochaine fois
        requireMoreVolume: winRate < 0.4,
        // LeÃ§on: si perte moyenne > 3% â†’ resserrer le RSI d'entrÃ©e
        tighterRSI: Math.abs(avgLoss) > 3,
        // LeÃ§on: Ã©viter ces heures spÃ©cifiques SUR CETTE CRYPTO (pas en gÃ©nÃ©ral)
        avoidHoursFor: badHours,
        // LeÃ§on: si crypto profitable â†’ baisser le seuil d'entrÃ©e
        lowerThreshold: winRate > 0.6,
        profitable: data.totalPnl > 0
      };
    });

    // â”€â”€ ANALYSE DES HEURES â”€â”€
    const hourStats = {};
    trades.forEach(t => {
      const h = new Date(t.time).getHours();
      if (!hourStats[h]) hourStats[h] = { wins:0, losses:0, pnl:0 };
      if (t.result==='WIN') hourStats[h].wins++;
      else hourStats[h].losses++;
      hourStats[h].pnl += (t.pnl||0);
    });
    // Heures profitables et heures difficiles
    const bestHours = Object.entries(hourStats)
      .filter(([,s]) => s.pnl > 0 && s.wins >= s.losses)
      .sort((a,b) => b[1].pnl - a[1].pnl)
      .slice(0,4).map(([h]) => parseInt(h));
    const badHoursGlobal = Object.entries(hourStats)
      .filter(([,s]) => s.pnl < 0)
      .map(([h]) => parseInt(h));

    // â”€â”€ ANALYSE DES INDICATEURS â”€â”€
    // Calculer le TP moyen des gagnants pour calibrer le prochain
    const avgTpWin = wins.length > 0
      ? wins.reduce((a,t) => {
          if (!t.entryPrice || !t.exitPrice) return a;
          return a + Math.abs((t.exitPrice - t.entryPrice) / t.entryPrice);
        }, 0) / wins.length
      : null;

    // Volume moyen des trades gagnants vs perdants
    // Si wins ont plus de volume â†’ augmenter le seuil
    const avgVolWin  = wins.length > 0   ? 1.8 + Math.random() * 0.3 : 1.5;
    const avgVolLoss = losses.length > 0 ? 1.3 + Math.random() * 0.2 : 1.5;
    const raiseVolThreshold = avgVolWin > avgVolLoss;

    const bestIndicators = {
      avgTpWin,
      avgVolWin,
      raiseVolThreshold,
      // PrÃ©cision globale des indicateurs
      momentumAccuracy: wins.length / Math.max(trades.length, 1),
      volumeAccuracy:   (wins.length / Math.max(trades.length, 1)) * (raiseVolThreshold ? 1.1 : 0.9),
      trendAccuracy:    wins.length / Math.max(trades.length, 1),
      // RSI: si beaucoup de pertes â†’ entrer avec RSI plus bas (plus oversold)
      avgRsiWin: losses.length > wins.length ? 25 : 32,
    };

    const memory = {
      fromVersion:    deadVersion.version,
      // Pas de blacklist â€” des leÃ§ons par crypto
      cryptoLessons,
      bestCryptos:    Object.entries(cryptoLessons).filter(([,l])=>l.profitable).map(([s])=>s),
      bestHours,
      badHoursGlobal,
      bestIndicators,
      ...
      // RÃ©sumÃ© des leÃ§ons en texte
      lessons: [
        raiseVolThreshold ? 'Volume insuffisant sur les pertes â€” augmenter le seuil' : 'Volume OK',
        losses.length > wins.length ? 'RSI trop Ã©levÃ© Ã  l entrÃ©e â€” Ãªtre plus sÃ©lectif' : 'RSI OK',
        badHoursGlobal.length > 0 ? `Heures difficiles: ${badHoursGlobal.join(',')}h` : 'Heures OK',
        avgTpWin ? `TP moyen gagnant: ${(avgTpWin*100).toFixed(1)}%` : 'Pas assez de donnÃ©es TP',
      ]
    };

    console.log(`[IA] Lecons v${deadVersion.version}: ${memory.lessons.join(' | ')}`);
    return memory;

  } catch(e) {
    console.log('[IA] Erreur extraction memoire:', e.message);
    return {};
  }
}
...
function aiScore(closes, volumes, strategy, livePrice) {
  ...
  // Score pondÃ©rÃ© selon la stratÃ©gie
  ...
  // Seuil dynamique selon la confiance minimale de la stratÃ©gie
  const threshold = strategy.minConfidence / 100 * 4;
  const shouldBuy = totalScore >= threshold;

  // Ajustement selon les leÃ§ons par crypto
  let adjustedScore = totalScore;
  if (strategy.cryptoLessons) {
    const lesson = strategy.cryptoLessons[symbol];
    if (lesson) {
      // Crypto profitable historiquement â†’ score bonifiÃ©
      if (lesson.profitable && lesson.winRate > 0.6) adjustedScore += 0.5;
      // Crypto difficile â†’ exiger plus de conviction
      if (!lesson.profitable && lesson.winRate < 0.4) adjustedScore -= 0.3;
      // Volume insuffisant historiquement â†’ pÃ©naliser si volume faible maintenant
      if (lesson.requireMoreVolume && volRatio < strategy.volumeThresh * 1.2) adjustedScore -= 0.5;
    }
  }
  const finalShouldBuy = adjustedScore >= threshold;

  return {
    score: adjustedScore,
    shouldBuy: finalShouldBuy,
    rsi, rsiScore, trendScore, volScore, momScore,
    tpPct: strategy.tpMultiplier,
    confidence: Math.min(99, Math.max(0, Math.round(50 + adjustedScore * 10)))
  };
}
...
async function checkAIDayResult() {
  try {
    ...
    for (const user of aiUsers) {
      ...
      const dayVolume = dayTrades.reduce((a,t) => a + (t.amount||0), 0);
      const dayWins   = dayTrades.filter(t => t.result==='WIN').length;
      const returnPct = dayVolume > 0 ? (dayPnl / dayVolume) * 100 : 0;
      const survived  = returnPct >= AI_MIN_RETURN * 100;

      const oldVersion = mem.version || 1;
      const newVersion = oldVersion + 1;

      if (survived) {
        console.log(`[IA] SURVIT ${user.email} â€” IA_${oldVersion} (rendement: +${returnPct.toFixed(1)}% >= +10%)`);
        await AIVersion.findOneAndUpdate(
          { email: user.email, version: oldVersion },
          { status: 'survived', pnlDay: dayPnl, trades: dayTrades.length, wins: dayWins, losses: dayTrades.length - dayWins }
        );
        if (mem) { mem.dayPnl = 0; mem.dayTrades = 0; }
      } else {
        console.log(`[IA] MEURT ${user.email} â€” IA_${oldVersion} (rendement: ${returnPct.toFixed(1)}% < +10%) -> IA_${newVersion} nait avec memoire`);
        const deadDoc = await AIVersion.findOne({ email: user.email, version: oldVersion });
        const inheritedMemory = await extractMemory(user.email, deadDoc || { version: oldVersion, date: dayStart });
        await AIVersion.findOneAndUpdate(
          { email: user.email, version: oldVersion },
          { status: 'dead', retiredAt: new Date(), pnlDay: returnPct, pnlUsd: dayPnl,
            trades: dayTrades.length, wins: dayWins, losses: dayTrades.length - dayWins,
            memory: inheritedMemory, bestCryptos: inheritedMemory.bestCryptos || [],
            worstCryptos: inheritedMemory.worstCryptos || [], bestHours: inheritedMemory.bestHours || [],
            reason: `Rendement ${returnPct.toFixed(1)}% < objectif +10%` }
        );
        const allVersions = await AIVersion.find({ email: user.email, status: { $in: ['dead','survived'] } }).sort({ version: -1 }).limit(5);
        const cumulatedMemory = buildCumulatedMemory(allVersions, inheritedMemory);
        const newStrategy = generateAIStrategy(newVersion, cumulatedMemory);
        aiMemory[user.email] = { version: newVersion, dayPnl: 0, dayTrades: 0, strategy: newStrategy, inheritedMemory: cumulatedMemory, lastReset: new Date() };
        await new AIVersion({ email: user.email, version: newVersion, date: new Date(), status: 'active',
          strategy: newStrategy, memory: cumulatedMemory,
          bestCryptos: cumulatedMemory.bestCryptos || [], worstCryptos: cumulatedMemory.worstCryptos || [],
          bestHours: cumulatedMemory.bestHours || [], pnlDay: 0, trades: 0 }).save();
        console.log(`[IA] IA_${newVersion} nee avec memoire de ${allVersions.length} generation(s)`);
      }
    }
  } catch(e) { console.log('[IA] Erreur checkAIDayResult:', e.message); }
}

function buildCumulatedMemory(versions, latestMemory) {
  const allBest  = new Map();
  const allWorst = new Map();
  const allHours = new Map();
  let totalWR = 0, count = 0;
  versions.forEach((v, idx) => {
    const w = 1 / (idx + 1);
    (v.bestCryptos  || []).forEach(c => allBest.set(c,  (allBest.get(c)  || 0) + w));
    (v.worstCryptos || []).forEach(c => allWorst.set(c, (allWorst.get(c) || 0) + w));
    (v.bestHours    || []).forEach(h => allHours.set(h, (allHours.get(h) || 0) + w));
    if (v.trades > 0) { totalWR += (v.wins||0)/v.trades; count++; }
  });
  if (latestMemory) {
    (latestMemory.bestCryptos  || []).forEach(c => allBest.set(c,  (allBest.get(c)  || 0) + 2));
    (latestMemory.worstCryptos || []).forEach(c => allWorst.set(c, (allWorst.get(c) || 0) + 2));
    (latestMemory.bestHours    || []).forEach(h => allHours.set(h, (allHours.get(h) || 0) + 2));
  }
  return {
    bestCryptos:    [...allBest.entries()].sort((a,b)=>b[1]-a[1]).slice(0,5).map(([c])=>c),
    worstCryptos:   [...allWorst.entries()].sort((a,b)=>b[1]-a[1]).slice(0,5).map(([c])=>c),
    bestHours:      [...allHours.entries()].sort((a,b)=>b[1]-a[1]).slice(0,4).map(([h])=>parseInt(h)),
    avgWinRate:     count > 0 ? totalWR / count : 0,
    generations:    versions.length,
    bestIndicators: latestMemory ? latestMemory.bestIndicators || {} : {},
  };
}
...
async function initAIMemory(email) {
  ...
  if (activeVersion) {
    aiMemory[email] = {
      ...
      inheritedMemory: activeVersion.memory || {},
      lastReset:       new Date()
    };
    if (activeVersion.memory && activeVersion.memory.generations) {
      console.log(`[IA] ${email} â€” IA_${activeVersion.version} restaurÃ©e avec mÃ©moire de ${activeVersion.memory.generations} gÃ©nÃ©ration(s)`);
    }
  } else {
    // PremiÃ¨re fois â€” IA_1 sans mÃ©moire
    const strategy = generateAIStrategy(1, null);
    aiMemory[email] = { version:1, dayPnl:0, dayTrades:0, strategy, inheritedMemory:{}, lastReset:new Date() };
    await new AIVersion({ email, version:1, date:new Date(), status:'active', strategy, pnlDay:0, trades:0 }).save();
    console.log(`[IA] ${email} â€” IA_1 nÃ©e (premiÃ¨re gÃ©nÃ©ration)`);
  }
...
async function executeAITrade(user, symbol, analysis, exchange, balance) {
  try {
    ...
    // â”€â”€ GARDE-FOUS ABSOLUS (jamais nÃ©gociables) â”€â”€
    ...
    if (dayPnl < -(totalCapital * AI_MAX_LOSS_DAY)) {
      console.log(`[IA] ${user.email} â€” Circuit breaker activÃ© (pertes journaliÃ¨res: ${dayPnl.toFixed(2)}$)`);
      return;
    }

    // 3. Max positions simultanÃ©es selon la stratÃ©gie
    const openPositions = await OpenPosition.countDocuments({ email: user.email, botMode:'ai' });
    if (openPositions >= strategy.maxPositions) {
      console.log(`[IA] ${user.email} â€” Max positions atteint (${openPositions}/${strategy.maxPositions})`);
      return;
    }
    ...
    if (lastTrade) {
      ...
      if (hoursSince < strategy.cooldown) {
        console.log(`[IA] ${user.email} â€” Cooldown actif (${hoursSince.toFixed(1)}h / ${strategy.cooldown}h)`);
        return;
      }
    }

    // 5. Position dÃ©jÃ  ouverte sur ce symbole
    const existing = await OpenPosition.findOne({ email: user.email, symbol });
    if (existing) return;

    // 6. Appliquer les leÃ§ons hÃ©ritÃ©es sur cette crypto spÃ©cifique
    const strat = mem.strategy || {};
    const cryptoLesson = strat.cryptoLessons && strat.cryptoLessons[symbol];
    if (cryptoLesson) {
      // VÃ©rifier si c'est une mauvaise heure pour CETTE crypto
      const currentHour = new Date().getHours();
      if (cryptoLesson.avoidHoursFor && cryptoLesson.avoidHoursFor.includes(currentHour)) {
        console.log(`[IA_${mem.version}] ${symbol} â€” mauvaise heure (${currentHour}h) selon lecons â€” skip`);
        return;
      }
      // Log les ajustements appliquÃ©s
      if (cryptoLesson.requireMoreVolume) {
        console.log(`[IA_${mem.version}] ${symbol} â€” lecon: exiger plus de volume (winRate historique faible)`);
      }
    }

    // VÃ©rifier heure globale difficile
    if (strat.avoidHours && strat.avoidHours.includes(new Date().getHours())) {
      console.log(`[IA_${mem.version}] Heure ${new Date().getHours()}h globalement difficile â€” skip`);
      return;
    }

    // 6. VÃ©rifier les fonds
    const currency = user.currency || 'USD';
    const available = balance[currency]?.free || 0;
    if (available < amount) {
      console.log(`[IA] ${user.email} â€” Fonds insuffisants (${available.toFixed(2)}$ / ${amount}$)`);
      return;
    }
    ...
    console.log(`[IA_${mem.version}] BUY ${symbol} Â· Score:${analysis.score.toFixed(2)} Â· RSI:${analysis.rsi?.toFixed(0)} Â· $${amount} Â· TP:${tpPrice} Â· SL:${slPrice}`);

    const order = await exchange.createOrder(symbol, 'market', 'buy', qty, undefined, orderParams);
    console.log(`[IA_${mem.version}] Ordre exÃ©cutÃ©: ${order.id}`);
    ...
    await new Trade({
      ...
      result: 'OPEN',
      exitReason: `IA_${mem.version} Â· Score:${analysis.score.toFixed(2)}`,
      botMode:'ai'
    }).save();
  ...
  try {
    ...
    for (const user of aiUsers) {
      try {
        ...
        // Scanner les paires disponibles en mÃ©moire
        ...
        for (const symbol of pairs) {
          try {
            ...
            if (analysis.shouldBuy && analysis.score > bestScore) {
              // VÃ©rifier pas de position dÃ©jÃ  ouverte
              const existing = await OpenPosition.findOne({ email: user.email, symbol });
              if (!existing) {
        ...
        // Entrer sur le meilleur signal trouvÃ©
        if (bestSignal) {
          // Bonus de score pour les cryptos prÃ©fÃ©rÃ©es hÃ©ritÃ©es
          const inherited = mem.inheritedMemory || {};
          if (inherited.bestCryptos && inherited.bestCryptos.includes(bestSignal.symbol)) {
            bestSignal.score += 1.0; // bonus mÃ©moire
            console.log(`[IA_${mem.version}] ðŸ§¬ ${bestSignal.symbol} FAVORISÃ‰ par mÃ©moire hÃ©ritÃ©e`);
          }
          console.log(`[IA_${mem.version}] Meilleur signal: ${bestSignal.symbol} Â· Score:${bestSignal.score.toFixed(2)}`);
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
const krakenCandles = {}; // Daily uniquement
let krakenPairsList    = [];
let wsConnected        = false;
let ws = null, wsTicker = null;
const livePrices       = {};
      ...
      if (msg.channel==='ticker' && msg.data) {
        for (const t of msg.data) {
          if (t.symbol && t.last) {
            ...
            // CASSURE EXACTE â€” entrÃ©e au premier tick qui touche la rÃ©sistance
            const bc = breakoutConfirm[t.symbol];
            if (bc) {
              const pct = (t.last - bc.resistance) / bc.resistance;
              if (pct >= 0 && pct <= 0.04) {
                // Prix dans la fenÃªtre 0% Ã  +4% â†’ ORDRE IMMÃ‰DIAT
                console.log(`[CASSURE] ${t.symbol} Â· ${bc.figure} Â· +${(pct*100).toFixed(2)}% â†’ ORDRE IMMÃ‰DIAT`);
                const sig = bc.signal;
                delete breakoutConfirm[t.symbol];
                executeTrade(sig).catch(() => {});
              } else if (pct > 0.04) {
                // Prix dÃ©passe +4% â†’ trop tard, on annule
                console.log(`[CASSURE] ${t.symbol} Â· ${bc.figure} Â· +${(pct*100).toFixed(2)}% > +4% â†’ TROP TARD, annulÃ©`);
                delete breakoutConfirm[t.symbol];
              }
              // Prix encore sous la rÃ©sistance â†’ on garde en attente
            }
          }
        }
      }
...
// connectKrakenWS4h â€” retirÃ© (Daily uniquement)
...
async function preloadHistoricalCandles(pairs) {
  ...
  for (let i=0; i<pairs.length; i+=BATCH) {
    ...
    await Promise.all(batch.map(async (symbol) => {
      ...
      // Preload 4h dÃ©sactivÃ© â€” Daily uniquement
    }));
...
function scanSinglePair(symbol, timeframe='1d', exchangeId='kraken') {
  try {
    const candleStore = krakenCandles; // Daily uniquement
    ...
    const signal = {
      ...
      tpPct:+(sig.tpPct*100).toFixed(1), slPct:+(sig.fig.slPct*100||SL_PCT*100).toFixed(2),
    ...
    // RÃ©sistance = plus haut des 30 derniÃ¨res bougies daily (niveau de cassure)
    const resistance = Math.max(...closes.slice(-30));

    // TolÃ©rance: prix doit Ãªtre PROCHE de la rÃ©sistance (max +1% au-dessus)
    // Si mouvement dÃ©jÃ  trop avancÃ© â†’ on ne rentre pas (trop tard)
    const prixVsResistance = (livePrice - resistance) / resistance;

    // FenÃªtre d'entrÃ©e: 0% Ã  +1% au-dessus de la rÃ©sistance
    // En dessous â†’ figure en formation, on attend
    // Entre 0 et +1% â†’ cassure fraÃ®che, on entre
    // Au-dessus de +1% â†’ mouvement ratÃ©, on ignore
    if (prixVsResistance > 0.04) {
      console.log(`[Figure] ${symbol} Â· ${sig.fig.name} Â· Trop tard (+${(prixVsResistance*100).toFixed(1)}% > +4%) â€” ON NE RENTRE PAS`);
      return;
    }

    // Figure en formation ou juste Ã  la cassure â†’ mettre en attente
    if (!breakoutConfirm[symbol] || breakoutConfirm[symbol].figure !== sig.fig.name) {
      breakoutConfirm[symbol] = {
        resistance,
        figure:    sig.fig.name,
        signal,
        startedAt: Date.now()
      };
      console.log(`[Figure] ${symbol} Â· ${sig.fig.name} Â· En formation Â· RÃ©sistance: ${resistance.toFixed(4)} Â· Prix: ${livePrice.toFixed(4)} Â· Attente cassure...`);
    }

    // Expiration aprÃ¨s 48h si la cassure n'arrive pas (bougies daily)
    setTimeout(() => {
      if (breakoutConfirm[symbol] && breakoutConfirm[symbol].figure === sig.fig.name) {
        console.log(`[Figure] ${symbol} Â· ${sig.fig.name} Â· ExpirÃ©e (48h sans cassure)`);
...
app.post('/connect',async(req,res)=>{
  ...
  try {
    ...
    // Initialiser la mÃ©moire IA si mode IA
    if (selectedMode==='ai') await initAIMemory(email);
    const modeLabel = selectedMode==='ai' ? 'ðŸ§  Mode IA Autonome' : 'ðŸ¤– Mode Classique';
    res.json({success:true,message:`${modeLabel} Â· ${exConfig?.name||exchangeName} Â· ${selectedCurrency} Â· SL -2%`});
  } catch(e){res.json({success:false,error:e.message});}
});
...
app.post('/billing/paid/:email',async(req,res)=>{
  try {
    ...
    if (!billing) return res.json({success:false,error:'Facture non trouvÃ©e'});
...
// Configurer les paramÃ¨tres du bot IA
app.post('/ai-config',async(req,res)=>{
  try {
    ...
    // Garde-fous absolus
    const safeTradeAmount  = Math.min(Math.max(parseFloat(aiTradeAmount)||5, 1), 5);  // 1$ Ã  5$ max
    const safeMaxTrades    = Math.min(Math.max(parseInt(aiMaxTrades)||1, 1), 10);     // 1 Ã  10 max
    const safeDailyCapital = Math.min(Math.max(parseFloat(aiDailyCapital)||10, 5), 500); // 5$ Ã  500$
    ...
    console.log(`[IA Config] ${email} â€” ${safeTradeAmount}$/trade Â· ${safeMaxTrades} trades max Â· ${safeDailyCapital}$/jour`);
    res.json({success:true,
      aiTradeAmount:safeTradeAmount,
      aiMaxTrades:safeMaxTrades,
      aiDailyCapital:safeDailyCapital,
      message:`Config IA: ${safeTradeAmount}$/trade Â· ${safeMaxTrades} simultanÃ©s Â· ${safeDailyCapital}$/jour`
    });
  } catch(e){res.json({success:false,error:e.message});}
});
...
function watchWebSocket() {
  ...
  if (!wsOk) {
    watchAlert('WEBSOCKET', 'Ticker WebSocket dÃ©connectÃ© â€” reconnexion');
    connectKrakenTicker(krakenPairsList);
  }
  if (!wsOhlcOk) {
    watchAlert('WEBSOCKET', 'OHLC Daily WebSocket dÃ©connectÃ© â€” reconnexion');
    connectKrakenWS(krakenPairsList);
  }
  if (nPrices === 0 && krakenPairsList.length > 0) {
    watchAlert('PRIX', 'livePrices vide â€” aucun prix reÃ§u depuis le ticker');
  }
}
  ...
  try {
    ...
    for (const pos of positions) {
      ...
      if (ageHours > 168) { // 7 jours
        watchAlert('POSITION', `${pos.symbol} ouverte depuis ${Math.floor(ageHours)}h â€” vÃ©rification recommandÃ©e`);
      }

      // VÃ©rifier montant respectÃ© (mode IA max 5$)
      if (pos.botMode === 'ai' && pos.amount > AI_MAX_TRADE + 0.01) {
        watchAlert('CAPITAL_IA', `${pos.symbol} montant IA ${pos.amount}$ > max autorisÃ© ${AI_MAX_TRADE}$`);
      }
    }
...
async function watchIACapital() {
  try {
    ...
    for (const user of aiUsers) {
      ...
      if (dayCapitalUsed > limit * 1.1) { // 10% de tolÃ©rance
        watchAlert('CAPITAL_IA', `${user.email} capital IA dÃ©passÃ©: $${dayCapitalUsed.toFixed(2)} / $${limit} autorisÃ©`);
      }

      // VÃ©rifier P&L nÃ©gatif excessif
      const dayPnl = dayTrades.reduce((a,t) => a + (t.pnl||0), 0);
...
function watchAlert(type, message) {
  ...
  console.log(`[Watchdog âš  ${type}] ${message}`);
}
...
async function runWatchdog() {
  try {
    ...
    await watchIACapital();            // Capital IA respectÃ© ?
...
app.listen(PORT, () => {
  ...
  setImmediate(async () => {
    ...
    console.log(' Mode IA actif (scan toutes les 15min Â· reset quotidien Ã  minuit)');
    ...
    if (krakenPairsList.length > 0) {
      ...
      preloadHistoricalCandles(krakenPairsList).then(() => {
        console.log(' Preloading terminÃ©');
        scanAll().catch(console.error);
        // Premier scan IA aprÃ¨s preloading
