...
// Schéma stratégies custom (Bot Builder)
const CustomStrategySchema = new mongoose.Schema({
  email:       String,
  name:        String,
  description: String,
  active:      { type: Boolean, default: true },
  indicators: {
    rsi:        { enabled: Boolean, period: Number, buyBelow: Number, sellAbove: Number },
    ema:        { enabled: Boolean, fast: Number, slow: Number, crossover: Boolean },
    volume:     { enabled: Boolean, minRatio: Number },
    resistance: { enabled: Boolean, lookback: Number },
    figure:     { enabled: Boolean, types: [String] }
  },
  entry:       { condition: String, timeframes: [String] },
  risk:        { slPct: Number, tpPct: Number, maxPositions: Number },
  stats:       { trades: { type: Number, default: 0 }, wins: { type: Number, default: 0 } },
  createdAt:   { type: Date, default: Date.now }
});
const CustomStrategy = mongoose.model('CustomStrategy', CustomStrategySchema);
...
function scanSinglePair(symbol, timeframe='1d', exchangeId='kraken', candleStoreOverride=null) {
  try {
    ...
    // Aussi évaluer les stratégies custom actives
    CustomStrategy.find({ active: true }).then(strategies => {
      for (const strat of strategies) {
        const tfs = strat.entry?.timeframes || ['1d'];
        if (!tfs.includes(timeframe)) continue;
        const customSig = evalCustomStrategy(strat, closes, volumes, livePrice);
        if (!customSig) continue;
        const customKey = symbol + '|' + strat._id.toString();
        if (breakoutConfirm[customKey]) continue;
        breakoutConfirm[customKey] = {
          resistance: Math.max(...closes.slice(-20)),
          figure: strat.name, signal: {
            symbol, exchange: 'Kraken', exchangeId: 'kraken',
            timeframe, market: 'Spot', figure: strat.name,
            figureCode: 'CUSTOM', direction: 'Long',
            entryPrice: livePrice, tp: customSig.tp, sl: customSig.sl,
            tpPct: +(customSig.tpPct * 100).toFixed(1),
            slPct: +(customSig.slPct * 100).toFixed(1),
            confidence: customSig.confidence,
            customStrategyId: strat._id.toString(),
            tradeAmount: TRADE_AMOUNT, time: new Date()
          }
        };
        console.log(`[Custom] ${symbol} · ${strat.name} · ${customSig.conditions}`);
        setTimeout(() => { delete breakoutConfirm[customKey]; }, 7*24*60*60*1000);
      }
    }).catch(() => {});
  } catch(e) {}
}
...
// ── Routes Bot Builder
app.post('/custom-strategy', async(req, res) => {
  try {
    const { email, strategy } = req.body;
    if (!email || !strategy || !strategy.name)
      return res.json({ success: false, error: 'Données manquantes' });

    // Sauvegarder ou mettre à jour la stratégie
    const saved = await CustomStrategy.findOneAndUpdate(
      { email, name: strategy.name },
      { ...strategy, email, active: true },
      { upsert: true, new: true }
    );
    console.log(`[Builder] ${email} → Stratégie "${strategy.name}" activée`);
    res.json({ success: true, id: saved._id, message: `Stratégie "${strategy.name}" activée!` });
  } catch(e) { res.json({ success: false, error: e.message }); }
});

app.get('/custom-strategies/:email', async(req, res) => {
  try {
    const strategies = await CustomStrategy.find({ email: req.params.email });
    res.json({ success: true, strategies });
  } catch(e) { res.json({ success: false, error: e.message }); }
});

app.delete('/custom-strategy/:id', async(req, res) => {
  try {
    await CustomStrategy.findByIdAndDelete(req.params.id);
    res.json({ success: true, message: 'Stratégie supprimée' });
  } catch(e) { res.json({ success: false, error: e.message }); }
});

app.post('/custom-strategy/toggle/:id', async(req, res) => {
  try {
    const strat = await CustomStrategy.findById(req.params.id);
    if (!strat) return res.json({ success: false, error: 'Non trouvée' });
    strat.active = !strat.active;
    await strat.save();
    res.json({ success: true, active: strat.active });
  } catch(e) { res.json({ success: false, error: e.message }); }
});
