...
function detectFigure(closes, volumes, livePrice, aiParams) {
  ...
  if (adx < 20) return null; // tendance trop faible

  const trend10 = closes[n-51] ? (price - closes[n-51]) / closes[n-51] : 0;

  // ── Helper: construire les niveaux depuis la HAUTEUR RÉELLE de la figure
  // figureTarget = potentiel de hausse calculé depuis la figure (min 40%)
  // TP = 25% de la hauteur projetée depuis le prix actuel
  function buildLevels(figureHigh, figureLow) {
    const figureHeight = figureHigh - figureLow;
    const figureTarget = figureHeight / figureLow; // potentiel réel de la figure
    if (figureTarget < 0.40) return null; // moins de 40% de potentiel → ignorer

    // TP selon la hauteur de la figure (projection technique)
    let tpPct;
    if      (figureTarget >= 0.80) tpPct = 0.20;
    else if (figureTarget >= 0.60) tpPct = 0.17;
    else if (figureTarget >= 0.50) tpPct = 0.15;
    else                           tpPct = 0.13;

    return {
      tp: +(price * (1 + tpPct)).toFixed(8),
      sl: +(price * (1 - SL_PCT)).toFixed(8),
      figureTarget,
      tpPct
    };
  }
  ...
  // ══ CUP & HANDLE ══
  // Hauteur = résistance - creux du cup
  if (n >= 100) {
    const cupLow     = Math.min(...closes.slice(n-80, n-20));
    const resistance = Math.max(...closes.slice(n-30, n-1));
    const levels     = buildLevels(resistance, cupLow);
    if (levels &&
        cupLow < closes[n-70] * 0.95 &&   // creux profond
        price > resistance * 0.98 &&        // prix près de la résistance
        volRatio > 1.8 &&
        trendBull && rsiOk)
      return { fig: FIGURES[0], ...levels };
  }

  // ══ ÉPAULE-TÊTE-ÉPAULE INVERSE ══
  // Hauteur = neckline - tête
  if (n >= 100) {
    const head        = Math.min(...closes.slice(n-60, n-20));
    const shoulderLow = Math.min(...closes.slice(n-80, n-60));
    const neckline    = Math.max(...closes.slice(n-80, n-2));
    const levels      = buildLevels(neckline, head);
    if (levels &&
        head < shoulderLow * 0.97 &&       // tête plus basse
        price > neckline * 0.98 &&          // prix près de la neckline
        volRatio > 1.5 &&
        rsiOk)
      return { fig: FIGURES[1], ...levels };
  }

  // ══ DOUBLE BOTTOM (W) ══
  // Hauteur = midTop - creux
  if (n >= 70) {
    ...
    const levels = buildLevels(midTop, botAvg);
    if (levels &&
        Math.abs(bot1-bot2)/bot1 < 0.03 && // creux similaires
        midTop > botAvg * 1.05 &&           // rebond significatif
        price > midTop * 0.98 &&            // prix près du midTop
        volRatio > 1.4 &&
        rsiOk)
      return { fig: FIGURES[2], ...levels };
  }

  // ══ TRIANGLE ASCENDANT ══
  // Hauteur = résistance - support (point d'entrée du triangle)
  if (n >= 40) {
    const resist30  = Math.max(...closes.slice(n-30, n));
    const support30 = Math.min(...closes.slice(n-30, n));
    const range     = (resist30 - support30) / price;
    const levels    = buildLevels(resist30, support30);
    if (levels &&
        range < 0.06 &&                    // range serré
        trend10 > 0.01 &&                  // tendance légèrement haussière
        price > resist30 * 0.995 &&        // prix qui teste la résistance
        volRatio > 1.6 &&
        trendBull && rsiOk)
      return { fig: FIGURES[3], ...levels };
  }

  // ══ DRAPEAU HAUSSIER ══
  // Hauteur = haut du mât - bas du drapeau (= potentiel de continuation)
  if (n >= 30) {
    const matStart = closes[n-25];
    const matTop   = Math.max(...closes.slice(n-20, n-8));
    const flagLow  = Math.min(...closes.slice(n-8,  n-1));
    const matGain  = (matTop - matStart) / matStart;
    const levels   = buildLevels(matTop + (matTop - flagLow), flagLow);
    if (levels &&
        matGain > 0.15 &&                  // mât de 15%+ (forte impulsion)
        (matTop - flagLow) / matTop < 0.12 && // retracement serré
        flagLow > matStart * 1.05 &&       // drapeau au-dessus du début du mât
        price > matTop * 0.98 &&           // prix qui casse le drapeau
        volRatio > 1.8 &&
        trend10 > 0.05)
      return { fig: FIGURES[4], ...levels };
  }

  // ══ BISEAU BAISSIER ══
  // Hauteur = haut du biseau - bas du biseau
  if (n >= 30) {
    const biseauHigh = Math.max(...closes.slice(n-25, n-1));
    const biseauLow  = Math.min(...closes.slice(n-25, n-1));
    const rsiRecent  = calcRSI(closes.slice(-20));
    const rsiOlder   = calcRSI(closes.slice(-40, -20));
    const levels     = buildLevels(biseauHigh, biseauLow);
    if (levels &&
        (biseauHigh - biseauLow) / biseauHigh < 0.08 && // compression
        trend10 < -0.01 && trend10 > -0.10 &&            // légèrement baissier
        rsiRecent > rsiOlder &&                           // divergence RSI haussière
        price > biseauHigh * 0.995 &&                    // cassure du biseau
        volRatio > 1.7)
      return { fig: FIGURES[5], ...levels };
  }
    ...
    await Promise.all(batch.map(async (symbol) => {
      try {
        ...
        if (d.length >= 10) {
          ...
          const recentVols = d.slice(-30).map(b => b.c * b.v).filter(v => v > 0);
          if (recentVols.length > 0)
            pairDailyVolume[symbol] = recentVols.reduce((a,b)=>a+b,0) / recentVols.length;
        }
      } catch(e) {}
      try {
...
function scanSinglePair(symbol, timeframe='1d', exchangeId='kraken', candleStoreOverride=null) {
  try {
    ...
    // Filtre prix minimum — évite les micro-cryptos incompatibles avec le montant minimum Kraken
    // Kraken minimum ~5$ par ordre → si prix trop bas, la qty dépasse le minimum d'unités
    if (livePrice < 0.001) return; // prix < 0.001$ → skip

    // Calculer volume si pas encore dans le cache
    if (!pairDailyVolume[symbol] && closes.length > 5) {
      const rv = candles.slice(-30).map(b => b.c * b.v).filter(v => v > 0);
      if (rv.length > 0) pairDailyVolume[symbol] = rv.reduce((a,b)=>a+b,0) / rv.length;
    }
    // Filtre volume quotidien minimum 500k$ — évite les cryptos manipulées
    const dailyVol = pairDailyVolume[symbol] || 0;
    if (dailyVol > 0 && dailyVol < MIN_DAILY_VOLUME_USD) return;
    ...
    if (!breakoutConfirm[bcKey] || breakoutConfirm[bcKey].figure !== sig.fig.name) {
      ...
      const vUSD = (pairDailyVolume[symbol]||0);
      console.log(`[Figure ${timeframe}] ${symbol} · ${sig.fig.name} · ${resistance.toFixed(4)} (${(pctAbove*100).toFixed(1)}%) · Vol:$${(vUSD/1000).toFixed(0)}k`);
    }
    setTimeout(() => {
  ...
  try {
    ...
    for (const user of users) {
      try {
        ...
        // Vérifier qty minimum Kraken avant l'ordre
        if (exchangeId === 'kraken') {
          try {
            const mkts = await exchange.loadMarkets();
            const mkt = mkts[signal.symbol];
            const minQty = mkt?.limits?.amount?.min || 0;
            if (minQty > 0 && qty < minQty) {
              console.log(`[Skip] ${signal.symbol} qty ${qty.toFixed(6)} < min Kraken ${minQty} (augmente montant/trade)`);
              continue;
            }
          } catch(e) {}
        }
        // Vérifier le volume minimum Kraken (évite "volume minimum not met")
        if (exchangeId === 'kraken') {
          try {
            const market = await exchange.loadMarkets();
            const mkt = market[signal.symbol];
            if (mkt && mkt.limits && mkt.limits.amount && mkt.limits.amount.min) {
              if (qty < mkt.limits.amount.min) {
                console.log(`[Skip] ${signal.symbol} qty ${qty.toFixed(6)} < min ${mkt.limits.amount.min} — augmente le montant/trade`);
                continue;
              }
            }
          } catch(e) {}
...
function scanKrakenFromMemory() {
  ...
  for (const symbol of krakenPairsList) {
    ...
    if (price < 0.001) continue; // prix trop bas → minimum Kraken non atteignable
...
app.listen(PORT, () => {
  ...
  setImmediate(async () => {
    ...
    // Diagnostic toutes les 5min
    setInterval(() => {
      const avecVol = Object.keys(pairDailyVolume).filter(k => pairDailyVolume[k] >= MIN_DAILY_VOLUME_USD).length;
      const avecCandles = Object.keys(krakenCandles).filter(k => (krakenCandles[k]||[]).length >= 100).length;
      const nbBC = Object.keys(breakoutConfirm).length;
      console.log(`[Diag] ${avecCandles} paires chargées · ${avecVol} qualifiées (>500k$) · ${nbBC} cassures en attente`);
    }, 5*60*1000);
