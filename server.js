...
const MIN_DAILY_VOLUME_USD = 500000; // 500k$ minimum par jour — évite les cryptos manipulées
...
// ADX > 25 = tendance forte = signal fiable
// ADX < 25 = marché en range = faux signal probable
// Nécessite les highs et lows (on approxime avec closes ±ATR)
function calcADX(closes, period=14) {
  if (closes.length < period * 2 + 1) return 25; // neutre si pas assez de données
  const n = closes.length;

  // Calculer True Range et DM à partir des closes
  // Approximation: high = close * 1.005, low = close * 0.995
  ...
  for (let i = 0; i < atr14.length; i++) {
    if (atr14[i] === 0) { dx.push(0); continue; }
    const pdi = 100 * plusDM14[i]  / atr14[i];
    const mdi = 100 * minusDM14[i] / atr14[i];
    const sum = pdi + mdi;
...
function detectFigure(closes, volumes, livePrice, aiParams) {
  ...
  // ADX — filtre anti-faux-signaux (force de la tendance)
  // ADX < 20 = marché en range = ignorer le signal
    ...
    if (msg.channel==='ohlc' && (msg.type==='snapshot'||msg.type==='update') && msg.data) {
      for (const c of msg.data) {
        ...
        if (!isNewCandle) {
          ...
          // Mettre à jour le volume quotidien en USD (Daily uniquement)
          if (store === krakenCandles && c.volume && c.close) {
            // Moyenne des 30 dernières bougies Daily pour avoir le volume quotidien moyen
            const recentVols = arr.slice(-30).map(b => b.c * b.v).filter(v => v > 0);
            if (recentVols.length > 0) {
              pairDailyVolume[sym] = recentVols.reduce((a,b)=>a+b,0) / recentVols.length;
            }
          }
    ...
    await Promise.all(batch.map(async (symbol) => {
      try {
        ...
        if (d.length >= 10) {
          ...
          // Calculer le volume quotidien moyen en USD
  ...
  try {
    ...
    for (const user of users) {
      try {
        ...
        if (available<amount) { console.log(`[Skip] Fonds insuffisants (${available.toFixed(2)}$ < ${amount}$)`); continue; }
...
function scanKrakenFromMemory() {
  ...
  for (const symbol of krakenPairsList) {
    ...
    // Filtre volume quotidien
    const dailyVol = pairDailyVolume[symbol] || 0;
    if (dailyVol < MIN_DAILY_VOLUME_USD) { filteredByVolume++; continue; }
...
async function scanAll() {
  ...
  try {
    ...
    console.log(`[Volume] ${Object.values(pairDailyVolume).filter(v=>v>=MIN_DAILY_VOLUME_USD).length} paires qualifiées (>${(MIN_DAILY_VOLUME_USD/1000).toFixed(0)}k$ vol/jour)`);
