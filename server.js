...
// Pour en ajouter une, il suffit d'ajouter une ligne ici -- le moteur est generique.
const MULTI_EXCHANGES = [
  { id: 'binance',  quote: 'USDT', maxPairs: 100 },
  { id: 'bybit',    quote: 'USDT', maxPairs: 100 },
  { id: 'coinbase', quote: 'USD',  maxPairs: 100 },
  // Ajouter ici : { id: 'okx', quote: 'USDT', maxPairs: 100 }, etc.
  // Plateformes supportees par ccxt (verifie) : bitget, okx, kucoin, gate, mexc,
  // bingx, phemex, htx, cryptocom, bitstamp, bitfinex, poloniex, deepcoin, xt,
  // toobit, lbank, deribit, woo, bitrue, ndax.
...
const POLL_INTERVAL_MS = 3 * 60 * 1000; // 3 minutes entre chaque cycle de sondage
...
// Cycle complet : pour chaque exchange configure, sonde le timeframe 1d
// (les autres timeframes peuvent etre ajoutes de la meme facon si besoin).
async function pollAllMultiExchanges() {
  for (const cfg of MULTI_EXCHANGES) {
    try {
      ...
      await pollExchangeTimeframe(cfg.id, multiExchangePairs[cfg.id], '1d', multiExchangeCandles[cfg.id]);
      console.log(`[Multi-${cfg.id}] Sondage 1d termine - ${Object.keys(multiExchangeCandles[cfg.id]['1d']||{}).length} paires en memoire`);
    } catch (e) {
      console.log(`[Multi-${cfg.id}] Erreur cycle:`, e.message);
    }
  }
}
...
function scanAllMultiExchanges(aiParams) {
  let all = [];
  for (const cfg of MULTI_EXCHANGES) {
    all = all.concat(scanMultiExchange(cfg.id, '1d', aiParams));
  }
  return all;
}
