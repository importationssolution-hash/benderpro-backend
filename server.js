...
function scanSinglePair(symbol, timeframe='1d', exchangeId='kraken', candleStoreOverride=null) {
  try {
    ...
    // ── Filtre volume quotidien — calculé depuis les candles si pas encore dans le cache
    if (!pairDailyVolume[symbol] && closes.length > 5) {
      const recentVols = candles.slice(-30).map(b => b.c * b.v).filter(v => v > 0);
    ...
    if (dailyVol > 0 && dailyVol < MIN_DAILY_VOLUME_USD) return; // paire trop petite
    ...
    if (!breakoutConfirm[bcKey] || breakoutConfirm[bcKey].figure !== sig.fig.name) {
      ...
      const volUSD = (pairDailyVolume[symbol]||0);
      console.log(`[Figure ${timeframe}] ${symbol} · ${sig.fig.name} · Résistance ${resistance.toFixed(4)} (${(pctAbove*100).toFixed(1)}%) · Vol: $${(volUSD/1000).toFixed(0)}k/j · ADX: ${calcADX(closes).toFixed(0)}`);
    }
    setTimeout(() => {
  ...
  try {
    ...
    for (const user of users) {
      try {
        ...
        // Filtre mémoire IA — éviter les figures qui ont causé des pertes
        const hour = new Date().getHours();
        if (!(await isFigureAllowed(user.email, signal.figure, hour))) continue;
...
app.listen(PORT, () => {
  ...
  setImmediate(async () => {
    ...
    // Diagnostic toutes les 5 minutes — voir pourquoi pas de trade
    setInterval(() => {
      const pairesAvecVolume = Object.keys(pairDailyVolume).filter(k => pairDailyVolume[k] >= MIN_DAILY_VOLUME_USD).length;
      const pairesAvecCandles = Object.keys(krakenCandles).filter(k => krakenCandles[k]?.length >= 100).length;
      const nbBreakouts = Object.keys(breakoutConfirm).length;
      console.log(`[Diagnostic] ${pairesAvecCandles} paires (${pairesAvecVolume} qualifiées vol) · ${nbBreakouts} cassures en attente · ${Object.keys(livePrices).length} prix live`);
    }, 5 * 60 * 1000);
